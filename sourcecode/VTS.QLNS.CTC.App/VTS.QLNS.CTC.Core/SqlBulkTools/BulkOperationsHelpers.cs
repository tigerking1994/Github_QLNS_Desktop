...
namespace VTS.QLNS.CTC.Core.SqlBulkTools
{
    internal class BulkOperationsHelpers
    {
        ...
        internal DataTable ToDataTable<T>(IEnumerable<T> items, HashSet<string> columns, Dictionary<string, string> columnMappings, Dictionary<string, object> columnDefaultValueMappings, List<string> matchOnColumns = null, bool? outputIdentity = null, Dictionary<int, T> outputIdentityDic = null)
        {
                    ...
                    if (column == "InternalId")
                    {
                        ...
                        for (int i = 0; i < props.Length; i++)
                        {
                            if (props[i].Name == column && item != null)
                            {
                                ...
                                if (columnDefaultValueMappings.ContainsKey(column))
                                {
                                    if (props[i].PropertyType.Equals(typeof(Guid)) && (value == null || Guid.Empty.Equals(value) ))
                                    {
                                        value = Guid.NewGuid();
                                    }
                                    else if (!props[i].PropertyType.Equals(typeof(Guid)) && value == null)
                                    {
                                        value = columnDefaultValueMappings[column];
                                    }
                                }
                                values.Add(value);
                            }
