...
{
    public class DanhMucNguonNganSachService : DanhMucService
    {
        private readonly IDanhMucRepository _danhMucRepository;

        public DanhMucNguonNganSachService(IDanhMucRepository danhMucRepository) : base(danhMucRepository)
        {
        ...
        {
            IEnumerable<DanhMuc> result = _danhMucRepository.FindAll(d => "NS_NguonNganSach".Equals(d.SType) && (d.INamLamViec == null || 
                d.INamLamViec == authenticationInfo.YearOfWork)).OrderBy(o=>o.IThuTu).ToList();
            return result;
        }
        ...
        public override IEnumerable<DanhMuc> FindFixNguonNganSach(Guid id)
        {
            return _danhMucRepository.FindAll(x => x.Id == id && "NS_NguonNganSach".Equals(x.SType)).OrderBy(x => x.IIDMaDanhMuc);
        }
    }
}
