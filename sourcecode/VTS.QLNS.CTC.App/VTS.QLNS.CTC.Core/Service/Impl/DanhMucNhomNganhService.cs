...
{
    public class DanhMucNhomNganhService : DanhMucService
    {
        private readonly IDanhMucRepository _danhMucRepository;

        public DanhMucNhomNganhService(IDanhMucRepository danhMucRepository) : base(danhMucRepository)
        {
        ...
        {
            IEnumerable<DanhMuc> result = _danhMucRepository.FindAll(d => "NS_Nganh_Nganh".Equals(d.SType) && d.INamLamViec == authenticationInfo.YearOfWork).ToList();
            foreach (DanhMuc d in result)
            {
                if (!String.IsNullOrEmpty(d.SGiaTri))
                {
                    string[] values = d.SGiaTri.Split(',').Select(p => p.Trim()).ToArray();
                    IEnumerable<DanhMuc> children = _danhMucRepository.FindAll(d => values.Contains(d.IIDMaDanhMuc) && d.INamLamViec == authenticationInfo.YearOfWork
                        && "NS_Nganh".Equals(d.SType)).ToList();
                    d.Values = children;
                }
            }
            return result;
