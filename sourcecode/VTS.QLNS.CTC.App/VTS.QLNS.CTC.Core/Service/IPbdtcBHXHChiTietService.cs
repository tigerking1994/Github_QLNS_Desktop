...
{
    public interface IPbdtcBHXHChiTietService
    {
        IEnumerable<BhPbdtcBHXHChiTiet> FindByCondition(Expression<Func<BhPbdtcBHXHChiTiet, bool>> predicate);

        int Add(BhPbdtcBHXHChiTiet item);
        int Update(BhPbdtcBHXHChiTiet item);
        int Delete(BhPbdtcBHXHChiTiet item);
        int AddRange(IEnumerable<BhPbdtcBHXHChiTiet> items);
        int RemoveRange(IEnumerable<BhPbdtcBHXHChiTiet> items);
        ...
        List<BhPbdtcBHXHChiBHXHReportQuery> GetListDataPhanBoLoaiChiKPQLKCBKHAC(int yearOfWork, string selectedUnits, Guid? iDLoaiChi, string sMaLoaiChi, string lstIDChungTu, int donViTinh, bool IsTongHopDonViKhoi, int dotNhanm, bool isMillionRound);
