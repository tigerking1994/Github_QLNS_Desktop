using System.Collections.Generic;
using System.Linq;
using VTS.QLNS.CTC.Utility.Enum;

namespace VTS.QLNS.CTC.App.Model.Import
{
    [SheetAttribute(0, "Chứng từ chi tiết", 6, 0)]
    public class AllocationImportModel : BindableBase
    {
        private bool _importStatus;
        public bool ImportStatus
        {
            get => _importStatus;
            set => SetProperty(ref _importStatus, value);
        }

        private bool _isWarning;
        public bool IsWarning
        {
            get => _isWarning;
            set => SetProperty(ref _isWarning, value);
        }

        private bool _bHangcha;
        public bool BHangCha
        {
            get => _bHangcha;
            set => SetProperty(ref _bHangcha, value);
        }

        public List<string> ListConcatenateCodeChild { get; set; }
        public string ConcatenateCodeParent { get; set; }

        private string _lns;
        [ColumnAttribute("LNS", 0)]
        public string LNS
        {
            get => _lns;
            set => SetProperty(ref _lns, value);
        }

        private string _l;
        [ColumnAttribute("L", 1)]
        public string L
        {
            get => _l;
            set => SetProperty(ref _l, value);
        }

        private string _k;
        [ColumnAttribute("K", 2)]
        public string K
        {
            get => _k;
            set => SetProperty(ref _k, value);
        }

        private string _m;
        [ColumnAttribute("M", 3)]
        public string M
        {
            get => _m;
            set => SetProperty(ref _m, value);
        }

        private string _tm;
        [ColumnAttribute("TM", 4)]
        public string TM
        {
            get => _tm;
            set => SetProperty(ref _tm, value);
        }

        private string _ttm;
        [ColumnAttribute("TTM", 5)]
        public string TTM
        {
            get => _ttm;
            set => SetProperty(ref _ttm, value);
        }

        private string _ng;
        [ColumnAttribute("NG", 6)]
        public string NG
        {
            get => _ng;
            set => SetProperty(ref _ng, value);
        }

        private string _xauNoiMa;
        //[ColumnAttribute("Xâu nối mã", 11)]
        public string XauNoiMa
        {
            get => string.Join("-", new string[] { _lns, _l, _k, _m, _tm, _ttm, _ng }.Where(s => !string.IsNullOrEmpty(s)));
            set => SetProperty(ref _xauNoiMa, value);
        }

        private string _moTa;
        [ColumnAttribute("Mô tả", 7)]
        public string MoTa
        {
            get => _moTa;
            set => SetProperty(ref _moTa, value);
        }

        private string _duToan;
        [ColumnAttribute("Dự toán", 9, ValidateType.IsNumber)]
        public string DuToan
        {
            get => _duToan;
            set => SetProperty(ref _duToan, value);
        }

        private string _daCap;
        [ColumnAttribute("Đã cấp", 10, ValidateType.IsNumber)]
        public string DaCap
        {
            get => _daCap;
            set => SetProperty(ref _daCap, value);
        }

        private string _deNghiDonVi;
        [ColumnAttribute("Đề nghị đơn vị", 11, ValidateType.IsNumber)]
        public string DeNghiDonVi
        {
            get => _deNghiDonVi;
            set => SetProperty(ref _deNghiDonVi, value);
        }

        private string _capPhat;
        [ColumnAttribute("Cấp phát", 12, ValidateType.IsNumber)]
        public string CapPhat
        {
            get => _capPhat;
            set => SetProperty(ref _capPhat, value);
        }

        private string _conLai;
        [ColumnAttribute("Còn lại", 13, ValidateType.IsNumber)]
        public string ConLai
        {
            get => _conLai;
            set => SetProperty(ref _conLai, value);
        }

        private string _ghiChu;
        [ColumnAttribute("Ghi chú", 14)]
        public string GhiChu
        {
            get => _ghiChu;
            set => SetProperty(ref _ghiChu, value);
        }

        private bool _isErrorMLNS;
        public bool IsErrorMLNS
        {
            get => _isErrorMLNS;
            set => SetProperty(ref _isErrorMLNS, value);
        }

        [ColumnAttribute(ValidateType.IsXauNoiMa)]
        public string ConcatenateCode
        {
            get => string.Join("-", new string[] { _lns, _l, _k, _m, _tm, _ttm, _ng }.Where(s => !string.IsNullOrEmpty(s)));
        }
    }
}
