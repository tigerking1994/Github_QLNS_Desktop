...
{
    public class BhQtcNamKinhPhiQuanLyChiTietModel : ModelBase
    {
        public Guid Id { get; set; }
        private Guid? _iID_QTC_Nam_KinhPhiQuanLy;

        public Guid? IID_QTC_Nam_KinhPhiQuanLy
        {
            get => _iID_QTC_Nam_KinhPhiQuanLy;
            set => SetProperty(ref _iID_QTC_Nam_KinhPhiQuanLy, value);
        }
        ...
        {
            get => FTien_DuToanNamTruocChuyenSang + FTien_DuToanGiaoNamNay;
            set
            {
        ...
        public double? FTienThua
        {
            get
            {
                return !string.IsNullOrEmpty(SDuToanChiTietToi) ? (FTien_TongDuToanDuocGiao > FTien_ThucChi ? FTien_TongDuToanDuocGiao - FTien_ThucChi : 0) : 0;
            }
        }
        public double? FTienThieu
        {
            get
            {
                return !string.IsNullOrEmpty(SDuToanChiTietToi) ? (FTien_TongDuToanDuocGiao < FTien_ThucChi ? FTien_ThucChi - FTien_TongDuToanDuocGiao : 0) : 0;
            }
        }
        ...
        public bool IsHasData => FTien_TongDuToanDuocGiao.GetValueOrDefault() != 0 || FTiLeThucHienTrenDuToan.GetValueOrDefault() != 0 || FTien_ThucChi.GetValueOrDefault() != 0 || FTien_DuToanNamTruocChuyenSang.GetValueOrDefault() != 0 || FTien_DuToanGiaoNamNay.GetValueOrDefault() != 0 || FTienThua.GetValueOrDefault() != 0 || FTienThieu.GetValueOrDefault() != 0;

        public bool IsHasDataChil
        {
            get
            {
                if (!IsHangCha)
                    return FTien_ThucChi.GetValueOrDefault(0) != 0 || FTien_DuToanGiaoNamNay.GetValueOrDefault(0) != 0;
                return false;
            }
        }
    }
