...
{
    public class DivisionImportViewModel : ViewModelBase
    {
        ...
        public Action<DtChungTuModel> SavedAction;
        public override string Name => "Dự toán";
        public override Type ContentType => typeof(DivisionImport);
        ...
        public DivisionImportViewModel(ISessionService sessionService,
            IMapper mapper,
        ...
        private void OnResetData()
        {
            ...
            _mucLucNganSachs = _nsMucLucNganSachService.FindAll(_sessionService.Current.YearOfWork, int.Parse(_cbxVoucherTypeSelected.ValueItem)).ToList();
        ...
        private void OnSaveData()
        {
            ...
            var chungTus = new List<NsDtChungTu>
            {
                new NsDtChungTu()
            };
            var divisionDetailsImport = _divisionDetails.Where(x => x.ImportStatus && !x.IsHangCha);

            var chungTuChiTiets = _mapper.Map<List<NsDtChungTuChiTiet>>(divisionDetailsImport);
            var lns = string.Join(StringUtils.COMMA, _divisionDetails.Where(x => x.ImportStatus).Select(x => x.LNS).ToHashSet());

            var soChungTuIndex = GetNextSoChungTuIndex();
            chungTus.ForEach(chungTu =>
            {
                chungTu.SSoChungTu = "DT-" + soChungTuIndex.ToString("D3");
                ...
                chungTu.SDslns = lns;
                chungTu.SDsidMaDonVi = _sessionService.Current.IdDonVi;
                ...
                chungTu.ILoai = SoChungTuType.ReceiveEstimate;

                chungTu.ILoaiChungTu = int.Parse(_cbxVoucherTypeSelected.ValueItem);
                chungTu.ILoaiDuToan = int.Parse(_cbxBudgetTypeSelected.ValueItem);

                chungTu.SNguoiTao = _sessionService.Current.Principal;
                chungTu.DNgayTao = DateTime.Now;
            });
            _chungTuService.AddRange(chungTus);

            var dictNsNganSachByXauNoiMa = ListNsMucLucNganSach.GroupBy(x => x.XauNoiMa)
                .ToDictionary(x => x.Key, x => x.First());
            var listChungTuChiTietResult = chungTus.SelectMany(chungTu =>
            {
                return chungTuChiTiets.Where(x => dictNsNganSachByXauNoiMa.ContainsKey(x.SXauNoiMa)).Select(chungTuChiTiet =>
                {
                    var targetItem = ObjectCopier.Clone(chungTuChiTiet);

                    targetItem.Id = Guid.Empty;
                    targetItem.IIdDtchungTu = chungTu.Id;
                    targetItem.BHangCha = false;

                    var nsMucLucNganSach = dictNsNganSachByXauNoiMa[chungTuChiTiet.SXauNoiMa];
                    targetItem.IIdMlns = nsMucLucNganSach.MlnsId;
                    targetItem.IIdMlnsCha = nsMucLucNganSach.MlnsIdParent;
                    targetItem.SL = nsMucLucNganSach.L;
                    targetItem.SK = nsMucLucNganSach.K;
                    targetItem.STng = nsMucLucNganSach.Tng;

                    targetItem.INamLamViec = _sessionService.Current.YearOfWork;
                    targetItem.INamNganSach = _sessionService.Current.YearOfBudget;
                    targetItem.IIdMaNguonNganSach = _sessionService.Current.Budget;
                    targetItem.IPhanCap = NSDuToan.IPHANCAP_NHAN_PHANBO;
                    targetItem.IDuLieuNhan = (int)DtDuLieuNhan.Import;

                    targetItem.SNguoiTao = _sessionService.Current.Principal;
                    targetItem.DNgayTao = DateTime.Now;

                    targetItem.SNguoiSua = _sessionService.Current.Principal;
                    targetItem.DNgaySua = DateTime.Now;

                    return targetItem;
                });
            }).ToList();
            _chungTuChiTietService.AddRange(listChungTuChiTietResult);
            ...
            var entityModel = _mapper.Map<DtChungTuModel>(chungTus.ElementAt(0));
