...
{
    public class AdjustedEstimateImportChosenViewModel : ViewModelBase
    {
        public override Type ContentType => typeof(AdjustedEstimateImportChosen);
        ...
        private ObservableCollection<ComboboxItem> _columnChosens;
        public ObservableCollection<ComboboxItem> ColumnChosens
        {
            get => _columnChosens;
            set => SetProperty(ref _columnChosens, value);
        }
        ...
        {
            ColumnChosens = new ObservableCollection<ComboboxItem>
            {
                new ComboboxItem {DisplayItem = "Dự toán ngân sách được giao", ValueItem = "0"},
                new ComboboxItem {DisplayItem = "Đã quyết toán đến thời điểm điều chỉnh", ValueItem = "1"}
            };
        }

        public AdjustedEstimateImportChosenViewModel(string messageContent)
            : this(messageContent, "Thông báo", NSMessageBoxButtons.OK, PackIconKind.CheckboxesMarkedCircle, null)
        {
        }

        public AdjustedEstimateImportChosenViewModel(string messageContent, Action<NSDialogResult> action)
            : this(messageContent, "Thông báo", NSMessageBoxButtons.OK, PackIconKind.CheckboxesMarkedCircle, action)
        {
        }

        public AdjustedEstimateImportChosenViewModel(string messageContent, string caption, Action<NSDialogResult> action)
            : this(messageContent, caption, NSMessageBoxButtons.OK, PackIconKind.CheckboxesMarkedCircle, action)
        {
        }

        public AdjustedEstimateImportChosenViewModel(string messageContent, string caption, NSMessageBoxButtons buttons, Action<NSDialogResult> action)
            : this(messageContent, caption, buttons, PackIconKind.HelpRhombus, action)
        {
        }

        public AdjustedEstimateImportChosenViewModel(string messageContent, string caption, NSMessageBoxButtons buttons, PackIconKind icon, Action<NSDialogResult> action)
        {
            Buttons = buttons;
        ...
        public AdjustedEstimateImportChosenViewModel(string messageContent, string caption, NSMessageBoxButtons buttons, PackIconKind icon, Action<NSDialogResult> action, Action func)
