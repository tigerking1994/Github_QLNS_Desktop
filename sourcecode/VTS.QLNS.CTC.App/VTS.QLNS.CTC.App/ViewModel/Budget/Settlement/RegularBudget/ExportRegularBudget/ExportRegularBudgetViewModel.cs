...
namespace VTS.QLNS.CTC.App.ViewModel.Budget.Settlement.RegularBudget.ExportDemandBudget
{
    public class ExportRegularBudgetViewModel : DialogViewModelBase<SettlementVoucherModel>
    {
        ...
        public override Type ContentType => typeof(View.Budget.Settlement.RegularBudget.ExportRegularBudget.ExportRegularBudget);
        ...
        private void LoadQuarterMonths()
        {
            ...
            if (TabIndex == ImportTabIndex.Aggregate)
            {
                if (_settlementVoucherSummaries.Any(w => w.Selected))
                {
                    ...
                    if (QuarterMonthValue == QuarterMonth.QUARTER)
                    {
                        var quarterMonthSelect = _settlementVoucherSummaries.FirstOrDefault(s => s.Selected).IThangQuy;
            ...
            else
            {
                if (_settlementVouchers.Any(w => w.Selected))
                {
                    ...
                    if (QuarterMonthValue == QuarterMonth.QUARTER)
                    {
                        var quarterMonthSelect = _settlementVouchers.FirstOrDefault(s => s.Selected).IThangQuy;
        ...
        {
            var month = SelectedQuarterMonth != null ? Int32.Parse(SelectedQuarterMonth.ValueItem) : 0;
            if (QuarterMonthValue == QuarterMonth.QUARTER)
            {
                if (month <= 3)
        ...
        public ExportRegularBudgetViewModel(ILog logger,
                IMapper mapper,
        ...
        private void OnExportAggregateDataByQuarter()
        {
            try
            {
                ...
                var iThangQuyEx = SelectedQuarterMonth != null ? Int32.Parse(SelectedQuarterMonth.ValueItem) : 0;
                if (TabIndex == ImportTabIndex.Aggregate)
                {
                    if (QuarterMonthValue == QuarterMonth.QUARTER)
                ...
                BackgroundWorkerHelper.Run((s, e) =>
                {
                    ...
                    if (TabIndex == ImportTabIndex.Aggregate)
                    {
                        if (QuarterMonthValue == QuarterMonth.QUARTER)
                        {
                            ...
                            var dataParentGroups = _settlementVoucherDetailExports.Where(x => x.BHangCha).GroupBy(g => new { g.SXauNoiMa, g.SMoTa }).Select(s => s.First()).ToList();
                            var dataChildGroups = _settlementVoucherDetailExports.Where(x => !x.BHangCha).GroupBy(g => new { g.IIdMlns, g.IIdMlnsCha, g.SXauNoiMa, g.SLns, g.SL, g.SK, g.SM, g.STm, g.STtm, g.SNg, g.STng, g.STng1, g.STng2, g.STng3, g.SMoTa, g.IsHangCha }).Select(x => new SettlementVoucherDetailModel
                            {
                            ...
                            var dataGroups = dataParentGroups;
        ...
        {
            var iIdMaDonViExport = AgencySelected.ValueItem;
            var iThangQuyEx = SelectedQuarterMonth != null ? Int32.Parse(SelectedQuarterMonth.ValueItem) : 0;
            ...
            if (QuarterMonthValue == QuarterMonth.QUARTER)
            {
                var listRemove = listChungTuChiTiet.Where(w => !w.BHangCha && !(w.IThangQuy >= ValueQuarter.FirstOrDefault() && w.IThangQuy <= ValueQuarter.LastOrDefault() && w.IIdMaDonVi == iIdMaDonViExport)).ToList();
                listChungTuChiTiet = listChungTuChiTiet.Except(listRemove).ToList();
            }
            else
            {
                var listRemove = listChungTuChiTiet.Where(w => !w.BHangCha && w.IThangQuy != iThangQuyEx).ToList();
                listChungTuChiTiet = listChungTuChiTiet.Except(listRemove).ToList();
            }
        ...
        private List<SettlementVoucherDetailModel> GetSettlementVoucherDetailByQuarter(List<SettlementVoucherModel> settlementVouchers)
        {
            ...
            int maxThangQuy = settlementVouchers.Max(x => x.IThangQuy);
            int minThangQuy = settlementVouchers.Min(x => x.IThangQuy);
            foreach (var item in settlementVouchers)
            {
                SettlementVoucherDetailSearch searchCondition = new SettlementVoucherDetailSearch
                {
                    ...
                    UserName = _sessionInfo.Principal,
                    QuarterMonth = item.IThangQuy.ToString(),
                };
                var listChungTuChiTietItem = _chungTuChiTietService.FindByCondition(searchCondition);
                var listRemove = listChungTuChiTietItem.Where(w => !w.BHangCha && !(w.IIdMaDonVi == iIdMaDonViExport)).ToList();
                listChungTuChiTietItem = listChungTuChiTietItem.Except(listRemove).ToList();
                listChungTuChiTiet.AddRange(listChungTuChiTietItem);

                if (item.IThangQuy != maxThangQuy) listChungTuChiTietItem.Select(x => x.FDuToan = 0).ToList();
                if (item.IThangQuy != minThangQuy) listChungTuChiTietItem.Select(x => x.FDaQuyetToan = 0).ToList();
            }
        ...
        private void CalculateData()
        {
            ...
            foreach (var item in _settlementVoucherDetailExports.Where(x => x.FDuToanOrigin != 0 || (x.IsEditable && (x.FDaQuyetToan != 0 || x.FTuChiPheDuyet != 0 || x.FTuChiDeNghi != 0 || x.FSoLuot != 0 || x.FSoNgay != 0 || x.FSoNguoi != 0 || x.FChuyenNamSauDaCap.GetValueOrDefault() != 0 || x.FChuyenNamSauChuaCap.GetValueOrDefault() != 0 || x.FDeNghiChuyenNamSau != 0))))
            {
        ...
        {
            var parentItem = _settlementVoucherDetailExports.Where(x => x.IIdMlns == currentItem.IIdMlnsCha).FirstOrDefault();
