...
namespace VTS.QLNS.CTC.App.ViewModel.Budget.Settlement.ExpenseBudget.ExportExpenseBudget
{
    public class ExportExpenseBudgetViewModel : DialogViewModelBase<SettlementVoucherModel>
    {
        ...
        public override Type ContentType => typeof(View.Budget.Settlement.ExpenseBudget.ExportExpenseBudget.ExportExpenseBudget);
        ...
        public ExportExpenseBudgetViewModel(ILog logger,
                IMapper mapper,
            ...
            try
            {
                BackgroundWorkerHelper.Run((s, e) =>
                {
                    ...
                    if (TabIndex == ImportTabIndex.Aggregate)
                    {
                        ...
                        {
                            List<SettlementVoucherDetailModel> settlementVoucherDetails = GetSettlementVoucherDetail(item);
                            ...
                            switch (BTieuChiSelected.ValueItem)
                            {
                                case nameof(MLNSFiled.NG):
                                    settlementVoucherDetails = settlementVoucherDetails.Where(x => string.IsNullOrEmpty(x.STng)).ToList();
                                    settlementVoucherDetails.Where(x => !string.IsNullOrEmpty(x.SNg)).Select(x => x.IsHangCha = false).ToList();
                                    break;
                                case nameof(MLNSFiled.TNG):
                                    settlementVoucherDetails = settlementVoucherDetails.Where(x => string.IsNullOrEmpty(x.STng1)).ToList();
                                    settlementVoucherDetails.Where(x => !string.IsNullOrEmpty(x.STng)).Select(x => x.IsHangCha = false).ToList();
                                    break;
                                case nameof(MLNSFiled.TNG1):
                                    settlementVoucherDetails = settlementVoucherDetails.Where(x => string.IsNullOrEmpty(x.STng2)).ToList();
                                    settlementVoucherDetails.Where(x => !string.IsNullOrEmpty(x.STng1)).Select(x => x.IsHangCha = false).ToList();
                                    break;
                                case nameof(MLNSFiled.TNG2):
                                    settlementVoucherDetails = settlementVoucherDetails.Where(x => string.IsNullOrEmpty(x.STng3)).ToList();
                                    settlementVoucherDetails.Where(x => !string.IsNullOrEmpty(x.STng2)).Select(x => x.IsHangCha = false).ToList();
                                    break;
                            }
