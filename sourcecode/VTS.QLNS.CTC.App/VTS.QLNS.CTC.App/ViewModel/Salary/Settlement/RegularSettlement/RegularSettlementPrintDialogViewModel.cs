...
{
    public class RegularSettlementPrintDialogViewModel : ViewModelBase
    {
        ...
        private string _typeChuKy = TypeChuKy.RPT_TL_QUYETTOAN_THUONGXUYEN;
        private TlQtChungTuChiTietGiaiThichModel _modelGiaiThich;
        private List<TlQtChungTuChiTietModel> _itemsChiTiet;
        ...
        public override Type ContentType => typeof(RegularSettlementPrintDialog);
        public bool IsExportEnable => _isData || _isDataInterpretation || _isVerbalExplanation || _isCover;
        public List<TlQtChungTuModel> ItemsChungTu { get; set; }
        public TlQtChungTuModel ChungTuModel { get; set; }
        ...
        private TlDmDonViModel _donviSelected;
        public TlDmDonViModel DonViSelected
        {
            get => _donviSelected;
            set
            {
        ...
        public RegularSettlementVerbalExplanationViewModel VerbalExplanationViewModel { get; set; }
        ...
        public RegularSettlementPrintDialogViewModel(
            ILog logger,
            IMapper mapper,
            ...
            RegularSettlementVerbalExplanationViewModel verbalExplanationViewModel,
        ...
        private void OnOpenDataInterpretationDialog()
        {
            ...
            // loc theo chung tu tong hop
            if (_bIsSummary)
                lstChungTu = lstChungTu.Where(x => !string.IsNullOrEmpty(x.STongHop));
            ...
            var results = _mapper.Map<List<TlQtChungTuModel>>(lstChungTu);
        ...
        private void OnVerbalExplanation()
        {
            ...
            VerbalExplanationViewModel.ChungTuModel = _mapper.Map<List<TlQtChungTuModel>>(lstChungTu).First();
        ...
        private ExportResult ExportCover()
        {
            ...
            string templateFileName = Path.Combine(ExportPrefix.PATH_TL_LUONG, ExportFileName.RPT_TL_TO_BIA_QTTX);
            string fileNameWithoutExtension = StringUtils.CreateExportFileName(fileNamePrefix);
            var xlsFile = _exportService.Export<TlQtChungTuChiTietModel>(templateFileName, dicData);
        ...
        public ExportResult ExportQtThuongXuyen(SalaryPrintType salaryPrintType = SalaryPrintType.BC_CHI_TIET_QUYET_TOAN_TX_SO_LIEU, ExportType exportType = ExportType.PDF)
        {
            ...
            List<TlQtChungTuChiTietModel> results = new List<TlQtChungTuChiTietModel>();
            List<TlQtChungTuChiTietModel> itemsChungTuChiTiet = FindChungTuChiTietByPrintType(salaryPrintType);
            if (string.IsNullOrEmpty(MaDonVi))
            {
                MessageBoxHelper.Error(Resources.MsgDonViEmpty);
                return null;
            }
            IEnumerable<TlDmDonVi> listDonVi = _tlDmDonViService.FindByDonViCon(MaDonVi);
            ...
            {
                List<TlQtChungTuChiTietModel> listDataDonViCon = new List<TlQtChungTuChiTietModel>();
                foreach (var donVi in listDonVi)
                {
                    ...
                    if (tlQtChungTu != null)
                    {
                        ...
                        var listDataItems = _mapper.Map<ObservableCollection<TlQtChungTuChiTietModel>>(listData);
                ...
                {
                    List<TlQtChungTuChiTietModel> donViCon = listDataDonViCon.Where(x => x.Lns == item.Lns && x.L == item.L && x.K == item.K && x.M == item.M && x.Tm == item.Tm && x.Ttm == item.Ttm && x.Ng == item.Ng).Select(x =>
                    {
                        x.IsParent = false;
                        x.MoTa = x.TenDonVi;
                        x.Lns = "";
            ...
            results = new List<TlQtChungTuChiTietModel>(results.OrderBy(x => x.XauNoiMa));
            ...
            AddChuKy(dicData, TypeChuKy.RPT_TL_QUYETTOAN_THUONGXUYEN);

            string templateFileName = Path.Combine(ExportPrefix.PATH_TL_LUONG, ExportFileName.RPT_TL_QT_THUONGXUYEN);
            string fileNameWithoutExtension = StringUtils.CreateExportFileName(fileNamePrefix);
            var xlsFile = _exportService.Export<TlQtChungTuChiTietModel>(templateFileName, dicData);
            return new ExportResult(fileNameWithoutExtension, fileNameWithoutExtension, null, xlsFile);
        }
        ...
        {
            var predicate = PredicateBuilder.True<TlDmDonViModel>();
            var lstChungTu = new List<TlQtChungTu>();
            if (!SNam.IsEmpty() && MonthSelected != null)
                lstChungTu = _tlQtChungTuService.FindAll(n => n.Nam.ToString() == SNam).ToList();
            List<TlDmDonViModel> lstDonVi = new List<TlDmDonViModel>();
            if (lstChungTu != null && lstChungTu.Any(n => !string.IsNullOrEmpty(n.MaDonVi)))
            {
                if (_bIsSummary)
                {
                    lstDonVi = lstChungTu.Where(n => !string.IsNullOrEmpty(n.MaDonVi) && !n.STongHop.IsEmpty()).Select(n => new TlDmDonViModel() { MaDonVi = n.MaDonVi, TenDonVi = n.TenDonVi }).ToList();
                }
                else
                {
                    lstDonVi = lstChungTu.Where(n => !string.IsNullOrEmpty(n.MaDonVi) && n.STongHop.IsEmpty()).Select(n => new TlDmDonViModel() { MaDonVi = n.MaDonVi, TenDonVi = n.TenDonVi }).ToList();
                ...
                DonViItems = new ObservableCollection<TlDmDonViModel>(_mapper.Map<List<TlDmDonViModel>>(lstDonVi));
            }
            else
            {
                DonViItems = null;
        ...
        private ExportResult OnExportDataInterpretation()
        {
            ...
            string templateFileName = Path.Combine(ExportPrefix.PATH_TL_LUONG, ExportFileName.RPT_TL_BAOCAO_GIAITHICH_SOLIEU);
            var xlsFile = _exportService.Export<TlQtChungTuChiTietGiaiThichModel>(templateFileName, data);
            string fileNamePrefix = "";
            if (iThang == 0)
            {
        ...
        private ExportResult OnExportVerbalExplaination(ExportType exportType)
        {
            ...
            string templateFileName = Path.Combine(ExportPrefix.PATH_TL_LUONG, ExportFileName.RPT_TL_BAOCAO_GIAITHICH_LOI);
            string fileNamePrefix = ExportFileName.RPT_TL_BAOCAO_GIAITHICH_LOI.Split(".").First() + "_" + TenDonVi;
            string fileNameWithoutExtension = StringUtils.CreateExportFileName(fileNamePrefix);
        ...
        public override void LoadData(params object[] args)
        {
            ...
            int iThang = int.Parse(MonthSelected.ValueItem);
            _itemsChiTiet = new List<TlQtChungTuChiTietModel>();
            List<TlQtChungTuChiTiet> lstLuyKeChiTiet = new List<TlQtChungTuChiTiet>();
            IEnumerable<TlQtChungTu> lstChungTuLuyKe = null;
            IEnumerable<TlQtChungTu> lstChungTu = _tlQtChungTuService.FindAll(n => n.Nam == Nam && n.MaDonVi == MaDonVi);
            if (lstChungTu != null)
            {
                switch (iThang)
                {
                ...
                if (!BIsDetailView)
                    ItemsChungTu = _mapper.Map<List<TlQtChungTuModel>>(lstChungTu);
            }
            if (ItemsChungTu == null || ItemsChungTu.Count == 0)
            {
                return;
            ...
            {
                var lstChungTuId = "";
                if (BIsSummary) lstChungTuId = string.Join(",", ItemsChungTu.Where(x => !string.IsNullOrEmpty(x.STongHop)).Select(n => n.STongHop));
                else lstChungTuId = string.Join(",", ItemsChungTu.Select(n => n.Id.ToString()));

                var data = _tlQtChungTuChiTietService.GetDataChungTuChiTietExport(string.Join(",", lstChungTuId), Nam, MaDonVi, BIsSummary);
                if (data != null)
                    _itemsChiTiet = _mapper.Map<List<TlQtChungTuChiTietModel>>(data);
            }
            if (lstChungTuLuyKe != null)
            {
                ...
                var lstCtChiTietLuyke = _tlQtChungTuChiTietService.FindAll(x => lstCtId.Contains(x.IdChungTu) && StringUtils.IsNullOrEmpty(x.MaCachTl));
            ...
            _modelGiaiThich = new TlQtChungTuChiTietGiaiThichModel();
        ...
        private void CalculateSettlementSalary()
        {
            ...
            if (BIsSummary) idChungTu = string.Join(",", ItemsChungTu.Where(x => !string.IsNullOrEmpty(x.STongHop)).Select(x => x.STongHop));
            else idChungTu = string.Join(",", ItemsChungTu.Select(x => x.Id.ToString()));
            var data = _tlQtChungTuChiTietService.GetDataGiaiThichBangSo(idChungTu, Nam, MaDonVi, BIsSummary).ToList();
            var itemsChungTuChiTiet = _mapper.Map<ObservableCollection<TlQtChungTuChiTietModel>>(data).ToList();
        ...
        private List<TlQtChungTuChiTietModel> FindChungTuChiTietByPrintType(SalaryPrintType printType)
        {
            List<TlQtChungTuChiTietModel> itemsChungTuChiTiet = new List<TlQtChungTuChiTietModel>();
            switch (printType)
            {
