...
{
    public class PheDuyetQuyetToanDAHTPrintDialogViewModel : DialogViewModelBase<NhQtPheDuyetQuyetToanDAHTModel>
    {
        private readonly ISessionService _sessionService;
        private readonly IMapper _mapper;
        private readonly ILog _logger;
        private readonly IDmChuKyService _dmChuKyService;
        private readonly string _typeChuky = TypeChuKy.RPT_NGOAIHOI_PHEDUYET_QUYETTOANDAHT;
        ...
        public override string Title => "PHÊ DUYỆT QUYẾT TOÁN DAHT";
        public override string Description => "Phê duyệt quyết toán DAHT";
        ...
        public PheDuyetQuyetToanDAHTPrintDialogViewModel(
        ...
        private void OnPrintReport(ExportType exportType)
        {
            ...
            try
            {
                BackgroundWorkerHelper.Run((s, e) =>
                {
                    ...
                    List<NhQtPheDuyetQuyetToanDAHTChiTietQuery> getData = new List<NhQtPheDuyetQuyetToanDAHTChiTietQuery>();
                    var dataDetail = _nhQtPheDuyetQuyetToanDAHTService.GetDataDetail(Model.IIdDonViId, Int32.Parse(SelectedDonViTinhUSD.ValueItem), Int32.Parse(SelectedDonViTinhVND.ValueItem), Model.Id).ToList();
                    if (dataDetail.Any())
                    {
                        getData = dataDetail;
                    }
                    ...
                    {
                        getData = _nhQtPheDuyetQuyetToanDAHTService.GetDataCreate(Model.INamBaoCaoTu ?? 0, Model.INamBaoCaoDen ?? 0, Model.IIdDonViId, Int32.Parse(SelectedDonViTinhUSD.ValueItem), Int32.Parse(SelectedDonViTinhVND.ValueItem)).ToList();
                    }
                    ...
                    var iTo = 0;
                    if (((11 + lstGiaiDoan.Count * 5) % _columnCountBC1) != 0)
                    {
                        iTo = lstGiaiDoan != null ? (((11 + lstGiaiDoan.Count * 5) / _columnCountBC1) + 1) : 1;

                    }
                    else
                    {
                        iTo = lstGiaiDoan != null ? (((11 + lstGiaiDoan.Count * 5) / _columnCountBC1)) : 1;
                    }
                    for (int i = 1; i <= iTo; i++)
                    {
                        string fileNamePrefixTo = string.Format("rptNgoaiHoi_PheDuyet_QuyetToan_Daht");
        ...
        public ExcelFile TaoFileExel(List<NhQtPheDuyetQuyetToanDAHTChiTietModel> data = null, List<NhTtThucHienNganSachGiaiDoanQuery> listGiaiDoan = null, int To = 1)
        {
            ...
            Result.Open(Path.Combine(_templatePath, ExportPrefix.PATH_NH_QT, ExportFileName.RPT_NGOAIHOI_PHEDUYET_QUYETTOAN_DAHT));
            ...
            var countColumn = 11 + listGiaiDoan.Count() + (listGiaiDoan.Count() * 2) + (listGiaiDoan.Count() * 2);
            ...
            if (listGiaiDoan != null)
            {
                ...
                {
                    var startColumn1 = 3 + listGiaiDoan.Count() + 1;
                    ...
                    if (i == 1)
                    {
                        ...
                        {
                            sTen = "Giá trị hợp đồng hoặc dự toán được duyệt",
                        });
            ...
            fr.SetValue(
                "TenBaoCao2", _dmChuKy != null ? _dmChuKy.TieuDe2MoTa?.ToUpper() : string.Empty
            );
            fr.SetValue(
               "TenBaoCao3", _dmChuKy != null ? _dmChuKy.TieuDe3MoTa?.ToUpper() : string.Empty
            );
            fr.SetValue(
                "To", To
            );
            ...
            foreach (var item in data)
            {
                ...
                item.lstData.Add(new NH_QT_PheDuyetQuyetToanDAHTDataGiaiDoan() { sGiaiDoan = StringUtils.DinhDangSo(item.FHopDongUsd, 2, false) });
                item.lstData.Add(new NH_QT_PheDuyetQuyetToanDAHTDataGiaiDoan() { sGiaiDoan = StringUtils.DinhDangSo(item.FHopDongVnd, 0, false) });
        ...
        private List<NhQtPheDuyetQuyetToanDAHTChiTietModel> GetList(List<NhQtPheDuyetQuyetToanDAHTChiTietModel> list, List<NhTtThucHienNganSachGiaiDoanQuery> lstGiaiDoan)
        {
            ...
            List<NHTHTongHop> lstDataThNVC = new List<NHTHTongHop>();
            int SttLoai = 0;
            int SttHopDong = 0;
            int SttDuAn = 0;
            ...
            if (lstGiaiDoan.Any())
            {
                ...
                {
                    var ItemsGiaiDoan = new List<int?> { giaiDoan.iGiaiDoanDen, giaiDoan.iGiaiDoanTu - 1 };
                    var lstDataTHCha = dataTongHops.Where(x => x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1 && x.INamKeHoach == giaiDoan.iGiaiDoanDen).ToList();

                    // nguon 306aj - 306ai-1
                    var dataThKinhPhiCapVnd = (lstDataTHCha.Where(x => x.SMaNguon == NhTongHopConstants.MA_306 && x.INamKeHoach == giaiDoan.iGiaiDoanDen).Sum(x => x.FGiaTriVnd) - lstDataTHCha.Where(x => x.SMaNguonCha == NhTongHopConstants.MA_306 && x.INamKeHoach == giaiDoan.iGiaiDoanDen).Sum(x => x.FGiaTriVnd)) - (lstDataTHCha.Where(x => x.SMaNguon == NhTongHopConstants.MA_306 && x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1).Sum(x => x.FGiaTriVnd) - lstDataTHCha.Where(x => x.SMaNguonCha == NhTongHopConstants.MA_306 && x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1).Sum(x => x.FGiaTriVnd)) ?? 0;

                    var dataThKinhPhiCapUsd = (lstDataTHCha.Where(x => x.SMaNguon == NhTongHopConstants.MA_306 && x.INamKeHoach == giaiDoan.iGiaiDoanDen).Sum(x => x.FGiaTriUsd) - lstDataTHCha.Where(x => x.SMaNguonCha == NhTongHopConstants.MA_306 && x.INamKeHoach == giaiDoan.iGiaiDoanDen).Sum(x => x.FGiaTriUsd)) - (lstDataTHCha.Where(x => x.SMaNguon == NhTongHopConstants.MA_306 && x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1).Sum(x => x.FGiaTriUsd) - lstDataTHCha.Where(x => x.SMaNguonCha == NhTongHopConstants.MA_306 && x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1).Sum(x => x.FGiaTriUsd)) ?? 0;
                    //dataTHKinhPhiDuocDuyet = nguon 301,304(aj) - nguon301,304(ai-1)
                    var dataThKinhPhiDuocDuyetUsd = (lstDataTHCha.Where(x => lstMaNguon.Contains(x.SMaNguon) && x.INamKeHoach == giaiDoan.iGiaiDoanDen).Sum(x => x.FGiaTriUsd) - lstDataTHCha.Where(x => lstMaNguon.Contains(x.SMaNguonCha) && x.INamKeHoach == giaiDoan.iGiaiDoanDen).Sum(x => x.FGiaTriUsd)) - (lstDataTHCha.Where(x => lstMaNguon.Contains(x.SMaNguon) && x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1).Sum(x => x.FGiaTriUsd) - lstDataTHCha.Where(x => lstMaNguon.Contains(x.SMaNguonCha) && x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1).Sum(x => x.FGiaTriUsd)) ?? 0;

                    var dataThKinhPhiDuocDuyetVnd = (lstDataTHCha.Where(x => lstMaNguon.Contains(x.SMaNguon) && x.INamKeHoach == giaiDoan.iGiaiDoanDen).Sum(x => x.FGiaTriUsd) - lstDataTHCha.Where(x => lstMaNguon.Contains(x.SMaNguonCha) && x.INamKeHoach == giaiDoan.iGiaiDoanDen).Sum(x => x.FGiaTriUsd)) - (lstDataTHCha.Where(x => lstMaNguon.Contains(x.SMaNguon) && x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1).Sum(x => x.FGiaTriUsd) - lstDataTHCha.Where(x => lstMaNguon.Contains(x.SMaNguonCha) && x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1).Sum(x => x.FGiaTriUsd)) ?? 0;
                    //----code cũ -----
                    ...
                    //DataTong.listDataTTCP.Add(new NH_QT_PheDuyetQuyetToanDAHTDataGiaiDoan() { valueUSD = lstDataTHCha.GroupBy(x => x.IIDKHTTNhiemVuChiID).Select(x => x.First()).Sum(x => x.FGiaTriUsd) });
                    DataTong.listDataKPDC.Add(new NH_QT_PheDuyetQuyetToanDAHTDataGiaiDoan() { valueUSD = dataThKinhPhiCapUsd, valueVND = dataThKinhPhiCapVnd });
                    DataTong.listDataQTDD.Add(new NH_QT_PheDuyetQuyetToanDAHTDataGiaiDoan() { valueUSD = dataThKinhPhiDuocDuyetUsd, valueVND = dataThKinhPhiDuocDuyetVnd });
                }
            }

            DataTong.FHopDongUsd = listTong.GroupBy(x => new { x.IIDDuAnId, x.IIDHopDongId, x.ILoaiNoiDungChi }).Select(x => x.First()).Sum(x => x.FHopDongUsdHopDong);
            DataTong.FHopDongVnd = listTong.GroupBy(x => new { x.IIDDuAnId, x.IIDHopDongId, x.ILoaiNoiDungChi }).Select(x => x.First()).Sum(x => x.FHopDongVndHopDong);
            ...
            DataTong.FKinhPhiDuocCapTongUsd = DataTong.listDataKPDC.Any() ? DataTong.listDataKPDC.Sum(x => x.valueUSD) : 0;
            DataTong.FKinhPhiDuocCapTongVnd = DataTong.listDataKPDC.Any() ? DataTong.listDataKPDC.Sum(x => x.valueVND) : 0;
            ...
            DataTong.FQuyetToanDuocDuyetTongUsd = DataTong.listDataQTDD.Any() ? DataTong.listDataQTDD.Sum(x => x.valueUSD) : 0;
            DataTong.FQuyetToanDuocDuyetTongVnd = DataTong.listDataQTDD.Any() ? DataTong.listDataQTDD.Sum(x => x.valueVND) : 0;
            ...
            if (list.Any())
            {
                foreach (var item in list)
                {
                        ...
                        if (lstGiaiDoan.Any())
                        {
                            foreach (var giaiDoan in lstGiaiDoan)
                            {
                                ...
                                if (item.INamBaoCaoTu == giaiDoan.iGiaiDoanTu && item.INamBaoCaoDen == giaiDoan.iGiaiDoanDen)
                                {
                                    ...
                                    item.listDataKPDC.Add(new NH_QT_PheDuyetQuyetToanDAHTDataGiaiDoan() { valueUSD = dataThKinhPhiCapUsd, valueVND = dataThKinhPhiCapVnd });
                                    item.listDataQTDD.Add(new NH_QT_PheDuyetQuyetToanDAHTDataGiaiDoan() { valueUSD = dataThKinhPhiDuocDuyetUsd, valueVND = dataThKinhPhiDuocDuyetVnd });
                                }
                                else
                                {
                    ...
                    if (item.IIDKHTTNhiemVuChiId != idChuongTrinh/* && item.IIDKHTTNhiemVuChiId != Guid.Empty*/)
                    {
                        ...
                        lstDataThNVC = dataTongHops.Where(x => x.IIDKHTTNhiemVuChiID == item.IIDKHTTNhiemVuChiId).ToList();
                        // nguon 306aj - 306ai-1
                        var dataThKinhPhiCapVnd = (lstDataThNVC.Where(x => x.SMaNguon == NhTongHopConstants.MA_306).Sum(x => x.FGiaTriVnd) - lstDataThNVC.Where(x => x.SMaNguonCha == NhTongHopConstants.MA_306).Sum(x => x.FGiaTriVnd)) - (lstDataThNVC.Where(x => x.SMaNguon == NhTongHopConstants.MA_306).Sum(x => x.FGiaTriVnd) - lstDataThNVC.Where(x => x.SMaNguonCha == NhTongHopConstants.MA_306).Sum(x => x.FGiaTriVnd)) ?? 0;

                        var dataThKinhPhiCapUsd = (lstDataThNVC.Where(x => x.SMaNguon == NhTongHopConstants.MA_306).Sum(x => x.FGiaTriUsd) - lstDataThNVC.Where(x => x.SMaNguonCha == NhTongHopConstants.MA_306).Sum(x => x.FGiaTriUsd)) - (lstDataThNVC.Where(x => x.SMaNguon == NhTongHopConstants.MA_306).Sum(x => x.FGiaTriUsd) - lstDataThNVC.Where(x => x.SMaNguonCha == NhTongHopConstants.MA_306).Sum(x => x.FGiaTriUsd)) ?? 0;
                        //dataTHKinhPhiDuocDuyet = nguon 301,304(aj) - nguon301,304(ai-1)
                        var dataThKinhPhiDuocDuyetUsd = (lstDataThNVC.Where(x => lstMaNguon.Contains(x.SMaNguon)).Sum(x => x.FGiaTriUsd) - lstDataThNVC.Where(x => lstMaNguon.Contains(x.SMaNguonCha)).Sum(x => x.FGiaTriUsd)) - (lstDataThNVC.Where(x => lstMaNguon.Contains(x.SMaNguon)).Sum(x => x.FGiaTriUsd) - lstDataThNVC.Where(x => lstMaNguon.Contains(x.SMaNguonCha)).Sum(x => x.FGiaTriUsd)) ?? 0;

                        var dataThKinhPhiDuocDuyetVnd = (lstDataThNVC.Where(x => lstMaNguon.Contains(x.SMaNguon)).Sum(x => x.FGiaTriUsd) - lstDataThNVC.Where(x => lstMaNguon.Contains(x.SMaNguonCha)).Sum(x => x.FGiaTriUsd)) - (lstDataThNVC.Where(x => lstMaNguon.Contains(x.SMaNguon)).Sum(x => x.FGiaTriUsd) - lstDataThNVC.Where(x => lstMaNguon.Contains(x.SMaNguonCha)).Sum(x => x.FGiaTriUsd));
                        ...
                        DataCha.FKinhPhiDuocCapTongUsd = dataThKinhPhiCapUsd;
                        DataCha.FKinhPhiDuocCapTongVnd = dataThKinhPhiCapVnd;
                        ...
                        DataCha.FQuyetToanDuocDuyetTongUsd = dataThKinhPhiDuocDuyetUsd;
                        DataCha.FQuyetToanDuocDuyetTongVnd = dataThKinhPhiDuocDuyetVnd;
                        ...
                        if (lstGiaiDoan.Any())
                        {
                            foreach (var giaiDoan in lstGiaiDoan)
                            {
                                ...
                                // nguon 306aj - 306ai-1
                                var dataThKinhPhiCapVndCt = (lstDataThNVC.Where(x => x.SMaNguon == NhTongHopConstants.MA_306 && x.INamKeHoach == giaiDoan.iGiaiDoanDen).Sum(x => x.FGiaTriVnd) - lstDataThNVC.Where(x => x.SMaNguonCha == NhTongHopConstants.MA_306 && x.INamKeHoach == giaiDoan.iGiaiDoanDen).Sum(x => x.FGiaTriVnd)) - (lstDataThNVC.Where(x => x.SMaNguon == NhTongHopConstants.MA_306 && x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1).Sum(x => x.FGiaTriVnd) - lstDataThNVC.Where(x => x.SMaNguonCha == NhTongHopConstants.MA_306 && x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1).Sum(x => x.FGiaTriVnd)) ?? 0;

                                var dataThKinhPhiCapUsdCt = (lstDataThNVC.Where(x => x.SMaNguon == NhTongHopConstants.MA_306 && x.INamKeHoach == giaiDoan.iGiaiDoanDen).Sum(x => x.FGiaTriUsd) - lstDataThNVC.Where(x => x.SMaNguonCha == NhTongHopConstants.MA_306 && x.INamKeHoach == giaiDoan.iGiaiDoanDen).Sum(x => x.FGiaTriUsd)) - (lstDataThNVC.Where(x => x.SMaNguon == NhTongHopConstants.MA_306 && x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1).Sum(x => x.FGiaTriUsd) - lstDataThNVC.Where(x => x.SMaNguonCha == NhTongHopConstants.MA_306 && x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1).Sum(x => x.FGiaTriUsd)) ?? 0;
                                //dataTHKinhPhiDuocDuyet = nguon 301,304(aj) - nguon301,304(ai-1)
                                var dataThKinhPhiDuocDuyetUsdCt = (lstDataThNVC.Where(x => lstMaNguon.Contains(x.SMaNguon) && x.INamKeHoach == giaiDoan.iGiaiDoanDen).Sum(x => x.FGiaTriUsd) - lstDataThNVC.Where(x => lstMaNguon.Contains(x.SMaNguonCha) && x.INamKeHoach == giaiDoan.iGiaiDoanDen).Sum(x => x.FGiaTriUsd)) - (lstDataThNVC.Where(x => lstMaNguon.Contains(x.SMaNguon) && x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1).Sum(x => x.FGiaTriUsd) - lstDataThNVC.Where(x => lstMaNguon.Contains(x.SMaNguonCha) && x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1).Sum(x => x.FGiaTriUsd)) ?? 0;

                                var dataThKinhPhiDuocDuyetVndCt = (lstDataThNVC.Where(x => lstMaNguon.Contains(x.SMaNguon) && x.INamKeHoach == giaiDoan.iGiaiDoanDen).Sum(x => x.FGiaTriUsd) - lstDataThNVC.Where(x => lstMaNguon.Contains(x.SMaNguonCha) && x.INamKeHoach == giaiDoan.iGiaiDoanDen).Sum(x => x.FGiaTriUsd)) - (lstDataThNVC.Where(x => lstMaNguon.Contains(x.SMaNguon) && x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1).Sum(x => x.FGiaTriUsd) - lstDataThNVC.Where(x => lstMaNguon.Contains(x.SMaNguonCha) && x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1).Sum(x => x.FGiaTriUsd)) ?? 0;
                                if (item.INamBaoCaoTu == giaiDoan.iGiaiDoanTu && item.INamBaoCaoDen == giaiDoan.iGiaiDoanDen)
                                {
                                    item.listDataTTCP.Add(new NH_QT_PheDuyetQuyetToanDAHTDataGiaiDoan() { valueUSD = item.FKeHoachTTCPUsd });
                                    item.listDataKPDC.Add(new NH_QT_PheDuyetQuyetToanDAHTDataGiaiDoan() { valueUSD = dataThKinhPhiCapUsdCt, valueVND = dataThKinhPhiCapVndCt });
                                    item.listDataQTDD.Add(new NH_QT_PheDuyetQuyetToanDAHTDataGiaiDoan() { valueUSD = dataThKinhPhiDuocDuyetUsdCt, valueVND = dataThKinhPhiDuocDuyetVndCt });
                                }
                                else
                                {
                    ...
                    NhQtPheDuyetQuyetToanDAHTChiTietModel DataChaDuAnNDC = new NhQtPheDuyetQuyetToanDAHTChiTietModel();

                    if (item.IIDDuAnId != idDuAn /*&& item.IIDDuAnId != Guid.Empty*/)
                    {
                        DataChaDuAnNDC = new NhQtPheDuyetQuyetToanDAHTChiTietModel();
                        SttDuAn++;
                        SttLoai = 0;
                        SttHopDong = 0;
                        idLoai = null;
                        ...
                        var dataThNhiemVuChiDuAn = lstDataThNVC.Where(x => x.IIdDuAnId == item.IIDDuAnId).ToList();
                        // nguon 306aj - 306ai-1
                        var dataThKinhPhiCapVnd = (dataThNhiemVuChiDuAn.Where(x => x.SMaNguon == NhTongHopConstants.MA_306).Sum(x => x.FGiaTriVnd) - dataThNhiemVuChiDuAn.Where(x => x.SMaNguonCha == NhTongHopConstants.MA_306).Sum(x => x.FGiaTriVnd)) - (dataThNhiemVuChiDuAn.Where(x => x.SMaNguon == NhTongHopConstants.MA_306).Sum(x => x.FGiaTriVnd) - dataThNhiemVuChiDuAn.Where(x => x.SMaNguonCha == NhTongHopConstants.MA_306).Sum(x => x.FGiaTriVnd)) ?? 0;

                        var dataThKinhPhiCapUsd = (dataThNhiemVuChiDuAn.Where(x => x.SMaNguon == NhTongHopConstants.MA_306).Sum(x => x.FGiaTriUsd) - dataThNhiemVuChiDuAn.Where(x => x.SMaNguonCha == NhTongHopConstants.MA_306).Sum(x => x.FGiaTriUsd)) - (dataThNhiemVuChiDuAn.Where(x => x.SMaNguon == NhTongHopConstants.MA_306).Sum(x => x.FGiaTriUsd) - dataThNhiemVuChiDuAn.Where(x => x.SMaNguonCha == NhTongHopConstants.MA_306).Sum(x => x.FGiaTriUsd)) ?? 0;
                        //dataTHKinhPhiDuocDuyet = nguon 301,304(aj) - nguon301,304(ai-1)
                        var dataThKinhPhiDuocDuyetUsd = (dataThNhiemVuChiDuAn.Where(x => lstMaNguon.Contains(x.SMaNguon)).Sum(x => x.FGiaTriUsd) - dataThNhiemVuChiDuAn.Where(x => lstMaNguon.Contains(x.SMaNguonCha)).Sum(x => x.FGiaTriUsd)) - (dataThNhiemVuChiDuAn.Where(x => lstMaNguon.Contains(x.SMaNguon)).Sum(x => x.FGiaTriUsd) - dataThNhiemVuChiDuAn.Where(x => lstMaNguon.Contains(x.SMaNguonCha)).Sum(x => x.FGiaTriUsd)) ?? 0;

                        var dataThKinhPhiDuocDuyetVnd = (dataThNhiemVuChiDuAn.Where(x => lstMaNguon.Contains(x.SMaNguon)).Sum(x => x.FGiaTriUsd) - dataThNhiemVuChiDuAn.Where(x => lstMaNguon.Contains(x.SMaNguonCha)).Sum(x => x.FGiaTriUsd)) - (dataThNhiemVuChiDuAn.Where(x => lstMaNguon.Contains(x.SMaNguon)).Sum(x => x.FGiaTriUsd) - dataThNhiemVuChiDuAn.Where(x => lstMaNguon.Contains(x.SMaNguonCha)).Sum(x => x.FGiaTriUsd)) ?? 0;
                        ...
                        if (lstGiaiDoan != null)
                        {
                            foreach (var giaiDoan in lstGiaiDoan)
                            {
                                List<NhQtPheDuyetQuyetToanDAHTChiTietModel> listDataChaGiaiDoan = listDataCha.Where(x => x.INamBaoCaoTu == giaiDoan.iGiaiDoanTu && x.INamBaoCaoDen == giaiDoan.iGiaiDoanDen).ToList();
                                // nguon 306aj - 306ai-1
                                var dataThKinhPhiCapVndCt = (dataThNhiemVuChiDuAn.Where(x => x.SMaNguon == NhTongHopConstants.MA_306 && x.INamKeHoach == giaiDoan.iGiaiDoanDen).Sum(x => x.FGiaTriVnd) - dataThNhiemVuChiDuAn.Where(x => x.SMaNguonCha == NhTongHopConstants.MA_306 && x.INamKeHoach == giaiDoan.iGiaiDoanDen).Sum(x => x.FGiaTriVnd)) - (dataThNhiemVuChiDuAn.Where(x => x.SMaNguon == NhTongHopConstants.MA_306 && x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1).Sum(x => x.FGiaTriVnd) - dataThNhiemVuChiDuAn.Where(x => x.SMaNguonCha == NhTongHopConstants.MA_306 && x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1).Sum(x => x.FGiaTriVnd)) ?? 0;

                                var dataThKinhPhiCapUsdCt = (dataThNhiemVuChiDuAn.Where(x => x.SMaNguon == NhTongHopConstants.MA_306 && x.INamKeHoach == giaiDoan.iGiaiDoanDen).Sum(x => x.FGiaTriUsd) - dataThNhiemVuChiDuAn.Where(x => x.SMaNguonCha == NhTongHopConstants.MA_306 && x.INamKeHoach == giaiDoan.iGiaiDoanDen).Sum(x => x.FGiaTriUsd)) - (dataThNhiemVuChiDuAn.Where(x => x.SMaNguon == NhTongHopConstants.MA_306 && x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1).Sum(x => x.FGiaTriUsd) - dataThNhiemVuChiDuAn.Where(x => x.SMaNguonCha == NhTongHopConstants.MA_306 && x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1).Sum(x => x.FGiaTriUsd)) ?? 0;
                                //dataTHKinhPhiDuocDuyet = nguon 301,304(aj) - nguon301,304(ai-1)
                                var dataThKinhPhiDuocDuyetUsdCt = (dataThNhiemVuChiDuAn.Where(x => lstMaNguon.Contains(x.SMaNguon) && x.INamKeHoach == giaiDoan.iGiaiDoanDen).Sum(x => x.FGiaTriUsd) - dataThNhiemVuChiDuAn.Where(x => lstMaNguon.Contains(x.SMaNguonCha) && x.INamKeHoach == giaiDoan.iGiaiDoanDen).Sum(x => x.FGiaTriUsd)) - (dataThNhiemVuChiDuAn.Where(x => lstMaNguon.Contains(x.SMaNguon) && x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1).Sum(x => x.FGiaTriUsd) - dataThNhiemVuChiDuAn.Where(x => lstMaNguon.Contains(x.SMaNguonCha) && x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1).Sum(x => x.FGiaTriUsd)) ?? 0;

                                var dataThKinhPhiDuocDuyetVndCt = (dataThNhiemVuChiDuAn.Where(x => lstMaNguon.Contains(x.SMaNguon) && x.INamKeHoach == giaiDoan.iGiaiDoanDen).Sum(x => x.FGiaTriUsd) - dataThNhiemVuChiDuAn.Where(x => lstMaNguon.Contains(x.SMaNguonCha) && x.INamKeHoach == giaiDoan.iGiaiDoanDen).Sum(x => x.FGiaTriUsd)) - (dataThNhiemVuChiDuAn.Where(x => lstMaNguon.Contains(x.SMaNguon) && x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1).Sum(x => x.FGiaTriUsd) - dataThNhiemVuChiDuAn.Where(x => lstMaNguon.Contains(x.SMaNguonCha) && x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1).Sum(x => x.FGiaTriUsd)) ?? 0;
                                if (item.INamBaoCaoTu == giaiDoan.iGiaiDoanTu && item.INamBaoCaoDen == giaiDoan.iGiaiDoanDen)
                                {
                                    item.listDataTTCP.Add(new NH_QT_PheDuyetQuyetToanDAHTDataGiaiDoan() { valueUSD = item.FKeHoachTTCPUsd });
                                    item.listDataKPDC.Add(new NH_QT_PheDuyetQuyetToanDAHTDataGiaiDoan() { valueUSD = dataThKinhPhiCapUsdCt, valueVND = dataThKinhPhiCapVndCt });
                    ...
                    if (item.IIDHopDongId != idHopDong && item.IIDHopDongId != Guid.Empty)
                    {
                        ...
                        var dataThNhiemVuChiDuAnHD = lstDataThNVC.Where(x => x.IIdHopDongId == item.IIDHopDongId && x.IIdDuAnId == item.IIDDuAnId).ToList();
                        // nguon 306aj - 306ai-1
                        var dataThKinhPhiCapVnd = (dataThNhiemVuChiDuAnHD.Where(x => x.SMaNguon == NhTongHopConstants.MA_306).Sum(x => x.FGiaTriVnd) - dataThNhiemVuChiDuAnHD.Where(x => x.SMaNguonCha == NhTongHopConstants.MA_306).Sum(x => x.FGiaTriVnd)) - (dataThNhiemVuChiDuAnHD.Where(x => x.SMaNguon == NhTongHopConstants.MA_306).Sum(x => x.FGiaTriVnd) - dataThNhiemVuChiDuAnHD.Where(x => x.SMaNguonCha == NhTongHopConstants.MA_306).Sum(x => x.FGiaTriVnd)) ?? 0;

                        var dataThKinhPhiCapUsd = (dataThNhiemVuChiDuAnHD.Where(x => x.SMaNguon == NhTongHopConstants.MA_306).Sum(x => x.FGiaTriUsd) - dataThNhiemVuChiDuAnHD.Where(x => x.SMaNguonCha == NhTongHopConstants.MA_306).Sum(x => x.FGiaTriUsd)) - (dataThNhiemVuChiDuAnHD.Where(x => x.SMaNguon == NhTongHopConstants.MA_306).Sum(x => x.FGiaTriUsd) - dataThNhiemVuChiDuAnHD.Where(x => x.SMaNguonCha == NhTongHopConstants.MA_306).Sum(x => x.FGiaTriUsd)) ?? 0;
                        //dataTHKinhPhiDuocDuyet = nguon 301,304(aj) - nguon301,304(ai-1)
                        var dataThKinhPhiDuocDuyetUsd = (dataThNhiemVuChiDuAnHD.Where(x => lstMaNguon.Contains(x.SMaNguon)).Sum(x => x.FGiaTriUsd) - dataThNhiemVuChiDuAnHD.Where(x => lstMaNguon.Contains(x.SMaNguonCha)).Sum(x => x.FGiaTriUsd)) - (dataThNhiemVuChiDuAnHD.Where(x => lstMaNguon.Contains(x.SMaNguon)).Sum(x => x.FGiaTriUsd) - dataThNhiemVuChiDuAnHD.Where(x => lstMaNguon.Contains(x.SMaNguonCha)).Sum(x => x.FGiaTriUsd)) ?? 0;

                        var dataThKinhPhiDuocDuyetVnd = (dataThNhiemVuChiDuAnHD.Where(x => lstMaNguon.Contains(x.SMaNguon)).Sum(x => x.FGiaTriUsd) - dataThNhiemVuChiDuAnHD.Where(x => lstMaNguon.Contains(x.SMaNguonCha)).Sum(x => x.FGiaTriUsd)) - (dataThNhiemVuChiDuAnHD.Where(x => lstMaNguon.Contains(x.SMaNguon)).Sum(x => x.FGiaTriUsd) - dataThNhiemVuChiDuAnHD.Where(x => lstMaNguon.Contains(x.SMaNguonCha)).Sum(x => x.FGiaTriUsd)) ?? 0;
                        ...
                        if (lstGiaiDoan != null)
                        {
                            foreach (var giaiDoan in lstGiaiDoan)
                            {
                                List<NhQtPheDuyetQuyetToanDAHTChiTietModel> listDataChaGiaiDoan = listDataCha.Where(x => x.INamBaoCaoTu == giaiDoan.iGiaiDoanTu && x.INamBaoCaoDen == giaiDoan.iGiaiDoanDen).ToList();
                                // nguon 306aj - 306ai-1
                                var dataThKinhPhiCapVndCt = (dataThNhiemVuChiDuAnHD.Where(x => x.SMaNguon == NhTongHopConstants.MA_306 && x.INamKeHoach == giaiDoan.iGiaiDoanDen).Sum(x => x.FGiaTriVnd) - dataThNhiemVuChiDuAnHD.Where(x => x.SMaNguonCha == NhTongHopConstants.MA_306 && x.INamKeHoach == giaiDoan.iGiaiDoanDen).Sum(x => x.FGiaTriVnd)) - (dataThNhiemVuChiDuAnHD.Where(x => x.SMaNguon == NhTongHopConstants.MA_306 && x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1).Sum(x => x.FGiaTriVnd) - dataThNhiemVuChiDuAnHD.Where(x => x.SMaNguonCha == NhTongHopConstants.MA_306 && x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1).Sum(x => x.FGiaTriVnd)) ?? 0;

                                var dataThKinhPhiCapUsdCt = (dataThNhiemVuChiDuAnHD.Where(x => x.SMaNguon == NhTongHopConstants.MA_306 && x.INamKeHoach == giaiDoan.iGiaiDoanDen).Sum(x => x.FGiaTriUsd) - dataThNhiemVuChiDuAnHD.Where(x => x.SMaNguonCha == NhTongHopConstants.MA_306 && x.INamKeHoach == giaiDoan.iGiaiDoanDen).Sum(x => x.FGiaTriUsd)) - (dataThNhiemVuChiDuAnHD.Where(x => x.SMaNguon == NhTongHopConstants.MA_306 && x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1).Sum(x => x.FGiaTriUsd) - dataThNhiemVuChiDuAnHD.Where(x => x.SMaNguonCha == NhTongHopConstants.MA_306 && x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1).Sum(x => x.FGiaTriUsd)) ?? 0;
                                //dataTHKinhPhiDuocDuyet = nguon 301,304(aj) - nguon301,304(ai-1)
                                var dataThKinhPhiDuocDuyetUsdCt = (dataThNhiemVuChiDuAnHD.Where(x => lstMaNguon.Contains(x.SMaNguon) && x.INamKeHoach == giaiDoan.iGiaiDoanDen).Sum(x => x.FGiaTriUsd) - dataThNhiemVuChiDuAnHD.Where(x => lstMaNguon.Contains(x.SMaNguonCha) && x.INamKeHoach == giaiDoan.iGiaiDoanDen).Sum(x => x.FGiaTriUsd)) - (dataThNhiemVuChiDuAnHD.Where(x => lstMaNguon.Contains(x.SMaNguon) && x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1).Sum(x => x.FGiaTriUsd) - dataThNhiemVuChiDuAnHD.Where(x => lstMaNguon.Contains(x.SMaNguonCha) && x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1).Sum(x => x.FGiaTriUsd)) ?? 0;

                                var dataThKinhPhiDuocDuyetVndCt = (dataThNhiemVuChiDuAnHD.Where(x => lstMaNguon.Contains(x.SMaNguon) && x.INamKeHoach == giaiDoan.iGiaiDoanDen).Sum(x => x.FGiaTriUsd) - dataThNhiemVuChiDuAnHD.Where(x => lstMaNguon.Contains(x.SMaNguonCha) && x.INamKeHoach == giaiDoan.iGiaiDoanDen).Sum(x => x.FGiaTriUsd)) - (dataThNhiemVuChiDuAnHD.Where(x => lstMaNguon.Contains(x.SMaNguon) && x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1).Sum(x => x.FGiaTriUsd) - dataThNhiemVuChiDuAnHD.Where(x => lstMaNguon.Contains(x.SMaNguonCha) && x.INamKeHoach == giaiDoan.iGiaiDoanTu - 1).Sum(x => x.FGiaTriUsd)) ?? 0;
                                if (item.INamBaoCaoTu == giaiDoan.iGiaiDoanTu && item.INamBaoCaoDen == giaiDoan.iGiaiDoanDen)
                                {
                                    item.listDataTTCP.Add(new NH_QT_PheDuyetQuyetToanDAHTDataGiaiDoan() { valueUSD = item.FKeHoachTTCPUsd });
                                    item.listDataKPDC.Add(new NH_QT_PheDuyetQuyetToanDAHTDataGiaiDoan() { valueUSD = dataThKinhPhiCapUsdCt, valueVND = dataThKinhPhiCapVndCt });
