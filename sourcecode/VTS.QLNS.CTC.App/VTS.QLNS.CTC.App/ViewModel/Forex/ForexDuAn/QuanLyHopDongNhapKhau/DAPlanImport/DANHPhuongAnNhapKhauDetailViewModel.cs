...
{
    public class DANHPhuongAnNhapKhauDetailViewModel : DialogCurrencyAttachmentViewModelBase<NhDaGoiThauModel>
    {
        ...
        /// <summary>
        /// list hạng mục của tất cả các chi phí cua nguồn vốn đang được chọn
        /// </summary>
        private ObservableCollection<NhDaGoiThauHangMucModel> tempListHangMuc = new ObservableCollection<NhDaGoiThauHangMucModel>();
        /// <summary>
        /// list chi phí của tất cả các nguồn vốn
        /// </summary>
        private ObservableCollection<NhDaGoiThauChiPhiModel> _tempListChiPhi = new ObservableCollection<NhDaGoiThauChiPhiModel>();
        /// <summary>
        /// keep tempListHangMuc khi SelectedGoiThauNguonVon thay đổi
        /// </summary>
        private Dictionary<Guid, ObservableCollection<NhDaGoiThauHangMucModel>> _dicNguonVonTempHangMuc = new Dictionary<Guid, ObservableCollection<NhDaGoiThauHangMucModel>>();
        /// <summary>
        /// keep _tempListChiPhi khi SelectedGoiThauNguonVon thay đổi
        /// </summary>
        private Dictionary<Guid, ObservableCollection<NhDaGoiThauChiPhiModel>> _dicNguonVonTempChiPhi = new Dictionary<Guid, ObservableCollection<NhDaGoiThauChiPhiModel>>();

        public override string Name => "THÔNG TIN GÓI THẦU CHI TIẾT";
        public override string Title => "Quản lý thông tin gói thầu chi tiết";
        public override Type ContentType => typeof(DANHPhuongAnNhapKhauDetail);
        ...
        public DANHPhuongAnNhapKhauItemDialogViewModel DANHPhuongAnNhapKhauItemDialogViewModel { get; }
        ...
        public DANHPhuongAnNhapKhauDetailViewModel
        (
            ...
            DANHPhuongAnNhapKhauItemDialogViewModel danhPhuongAnNhapKhauItemDialogViewModel
        ) : base(mapper, nhDmTiGiaService, nhDmTiGiaChiTietService, storageServiceFactory, attachService)
        {
            ...
            DANHPhuongAnNhapKhauItemDialogViewModel = danhPhuongAnNhapKhauItemDialogViewModel;
            DANHPhuongAnNhapKhauItemDialogViewModel.ParentPage = this;
            ...
            _dicNguonVonTempHangMuc = new Dictionary<Guid, ObservableCollection<NhDaGoiThauHangMucModel>>();
            _dicNguonVonTempChiPhi = new Dictionary<Guid, ObservableCollection<NhDaGoiThauChiPhiModel>>();
        }
        ...
        private void LoadGoiThauChiPhi(NhDaGoiThauNguonVonModel nguonVon)
        {
            ...
            else if (IsAdded)
            {
                ...
                if (!NhHdnkPhuongAnNhapKhauModel.IIdDuAnId.IsNullOrEmpty() && SLoaiSoCu != SO_CU_TRUC_TIEP.CHUONG_TRINH)
                {
                    if (SLoaiSoCu == SO_CU_TRUC_TIEP.QUYET_DINH_DAU_TU)
                    {
                        ...
                        nguonVon.GoiThauChiPhis = _itemsChiPhi;
                    }
                }
            }
            else
            {
                ...
                if (!data.IsEmpty())
                {
                    ...
                    // cập nhật list chi phí này vào ItemsGoiThauNguonVon
                    var currentNguonVon = ItemsGoiThauNguonVon.FirstOrDefault(x => x.Id == nguonVon.Id);
                    if (dataMap != null)
                    {
                        dataMap.ForAll(s =>
                        {
                            s.STenNguonVon = nguonVon.STenNguonVon;
                            s.ItemsLoaiNoiDungChi = new ObservableCollection<NhDmChiPhiModel>(ItemsDMChiPhi);
                            s.PropertyChanged += GoiThauChiPhi_PropertyChanged;
                            s.GoiThauHangMucs = _mapper.Map<ObservableCollection<NhDaGoiThauHangMucModel>>(LoadNhDaGoiThauHangMucs(s));
                        ...
                        currentNguonVon.GoiThauChiPhis = new ObservableCollection<NhDaGoiThauChiPhiModel>(dataMap);
                        SelectedGoiThauNguonVon.GoiThauChiPhis = new ObservableCollection<NhDaGoiThauChiPhiModel>(dataMap);
                        OnPropertyChanged(nameof(ItemsChiPhi));
                    }
                }
            }
        ...
        private void OnAddGoiThauChiPhi(object obj)
        {
            ...
            var rawGoiThauChiPhis = ItemsGoiThauNguonVon.FirstOrDefault(m => m.Id == SelectedGoiThauNguonVon.Id)?.GoiThauChiPhis;
        ...
        {
            DANHPhuongAnNhapKhauItemDialogViewModel.ShowDialogHost("NHPhuongAnNhapKhauItemDialog");
                ...
                case SO_CU_TRUC_TIEP.THONG_TIN_DU_AN:
                    ...
                    if (_ttdaDuanHangMuc != null && _ttdaDuanHangMuc.Count > 0)
                    {
                        ...
                        foreach (var item in _ttdaDuanHangMuc.Keys)
                        {
                            ...
                            DANHPhuongAnNhapKhauItemDialogViewModel.OnAddGoiThauHangMuc(new NhDaGoiThauHangMucModel
                            {
        ...
        //public void NguonVonDataGrid_SelectionChanged()
        //{
        //    if (SelectedGoiThauNguonVon != null)
        //    {
        //        ItemsChiPhi = new ObservableCollection<NhDaGoiThauChiPhiModel>(SelectedGoiThauNguonVon.GoiThauChiPhis);
        //    }
        //}
        ...
        private void GoiThauNguonVon_PropertyChanged(object sender, PropertyChangedEventArgs args)
        {
            ...
            if (args.PropertyName == nameof(NhDaGoiThauNguonVonModel.IsChecked))
            {
                ...
                else // khi bỏ chọn nguồn vốn
                {
                    ...
                    // cho vào _itemsChiPhiTemp, nếu sau này check lại nguồn vốn này thì item lại sẽ lại xuất hiện -> Bỏ 
                    ...
                    // bỏ chọn là xóa, k giữ lại gì hết
                    var chiPhisOfUncheckedNguonVon = ItemsGoiThauNguonVon.FirstOrDefault(m => m.Id == SelectedGoiThauNguonVon.Id).GoiThauChiPhis;
                    chiPhisOfUncheckedNguonVon.Clear();
                    ItemsChiPhi.Clear();
                }
        ...
        public void LoadDataHangMuc()
        {
            ...
            //        IEnumerable<Guid?> listHangMucAssignedToOtherChiPhi = tempListHangMuc.Where(h => h.IIdGoiThauChiPhiId != SelectedChiPhi.Id && (h.FGiaTriUsd > 0 || h.FGiaTriVnd > 0)).Select(h => h.IIdQdDauTuHangMucId);

            //        // giữ lại các hạng mục đã được assign giá trị
            //        tempListHangMuc = new ObservableCollection<NhDaGoiThauHangMucModel>(tempListHangMuc.Where(x => x.FGiaTriUsd > 0 || x.FGiaTriVnd > 0));
            ...
            //            {
            //                // nếu hạng mục này mà chưa được điền giá trị ứng với 1 chi phí khác thì mới add
            //                if (!listHangMucAssignedToOtherChiPhi.Contains(s.Id))
            //                {
            //                    tempListHangMuc.Add(new NhDaGoiThauHangMucModel
            //                    {
            //                        Id = Guid.NewGuid(),
