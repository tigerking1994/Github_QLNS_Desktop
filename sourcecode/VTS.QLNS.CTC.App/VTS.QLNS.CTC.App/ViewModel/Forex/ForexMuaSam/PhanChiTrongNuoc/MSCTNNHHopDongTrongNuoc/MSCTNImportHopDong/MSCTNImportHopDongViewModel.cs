...
namespace VTS.QLNS.CTC.App.ViewModel.Forex.ForexMuaSam.PhanChiTrongNuoc.MSCTNNHHopDongTrongNuoc.MSCTNImportHopDong

{
    public class MSCTNImportHopDongViewModel : ViewModelBase
    {
        ...
        public override Type ContentType => typeof(View.Forex.ForexMuaSam.PhanChiTrongNuoc.MSCTNNHHopDongTrongNuoc.ImportHopDong.ImportHopDong);
        ...
        private ObservableCollection<HopDongTrongNuocDuAnImportModel> _hopDongImportModels;
        public ObservableCollection<HopDongTrongNuocDuAnImportModel> HopDongImportModels
        {
        ...
        public MSCTNNHHopDongTrongNuocDialogViewModel NHHopDongTrongNuocDialogViewModel { get; set;}
        ...
        public MSCTNImportHopDongViewModel(ISessionService sessionService,
            ILog logger,
            IMapper mapper,
            ...
            MSCTNNHHopDongTrongNuocDialogViewModel nhHopDongTrongNuocDialogViewModel)
        {
        ...
        private void OnResetData()
        {
            ...
            HopDongImportModels = new ObservableCollection<HopDongTrongNuocDuAnImportModel>();
        ...
        private void OnProcessFile()
        {
            ...
            {
                _importErrors.Clear();
                ImportResult<HopDongTrongNuocDuAnImportModel> dataImport = _importService.ProcessData<HopDongTrongNuocDuAnImportModel>(FileName);
                HopDongImportModels = new ObservableCollection<HopDongTrongNuocDuAnImportModel>(dataImport.Data);
        ...
        private List<NhDaHopDongModel> ConvertData(List<HopDongTrongNuocDuAnImportModel> importModels, bool isAddMessage = true)
        {
            ...
            IEnumerable<NhDaDuAnQuery> nhDaDuAns = _nhDaDuAnService.FindIndex(1).Where(x => x.IIdDonViQuanLyId == SelectedDonVi.Id);
            ...
            int i = 0;
            foreach (HopDongTrongNuocDuAnImportModel import in importModels)
            {
                ...
                {
                    NhDmLoaiHopDong loaiHopDong = nhDmLoaiHopDongs.FirstOrDefault(x => x.SMaLoaiHopDong.ToUpper().Equals(import.MaLoaiHopDong?.ToUpper()));
                    if (loaiHopDong != null)
                    {
                ...
                {
                    NhDmNhaThau nhaThau = nhDmNhaThaus.FirstOrDefault(x => x.SMaNhaThau.ToUpper().Equals(import.MaNhaThauDaiDien?.ToUpper()));
                    if (nhaThau != null)
                    {
                ...
                it.SMaDuAn = import.MaDuAn;
                if (!string.IsNullOrEmpty(import.MaDuAn))
                {
                    NhDaDuAnQuery duAn = nhDaDuAns.FirstOrDefault(x => x.SMaDuAn.Equals(import.MaDuAn));
                    if (duAn != null)
                    {
                        it.SMaDuAn = duAn.SMaDuAn;
                        it.IIdDuAnId = duAn.Id;
                    }
                    else
                    {
                        it.ImportStatus = false;
                        if (isAddMessage) _importErrors.Add(new ImportErrorItem { Row = i, Error = Resources.MsgNotExistData, ColumnName = "Mã dự án" });
                    }
                }
