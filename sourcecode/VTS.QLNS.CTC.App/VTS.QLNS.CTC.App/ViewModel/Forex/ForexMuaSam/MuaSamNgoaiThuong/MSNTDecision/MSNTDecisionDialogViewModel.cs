...
{
    public class MSNTDecisionDialogViewModel : DialogCurrencyAttachmentViewModelBase<NhQuyetDinhDamPhamModel>
    {
        ...
        public override Type ContentType => typeof(MSNTDecisionDialog);
        ...
        public MSNTDecisionDialogDetailViewModel MSNTDecisionDialogDetailViewModel { get; }

        public MSNTDecisionDialogViewModel
        (
            ILog logger,
            IMapper mapper,
            ...
            MSNTDecisionDialogDetailViewModel msntdecisionDialogDetailViewModel
        )
        ...
        private void LoadPhuongAnNhapKhau()
        {
            ...
            if (IsViewDuAn)
            {
                // Nếu hiển thị dự án, thì load PANK theo dự án
                if (_selectedDuAn != null)
                {
                    IEnumerable<NhHdnkPhuongAnNhapKhau> data = _phuongAnNhapKhauService.FindIndex().Where(s => s.IIdDuAnId == _selectedDuAn.Id && s.BIsActive==true && s.ILoai == 1);
            ...
            else
            {
                ...
                if (_selectedChuongTrinh != null)
                {
                    IEnumerable<NhHdnkPhuongAnNhapKhau> data = _phuongAnNhapKhauService.FindIndex().Where(s => s.IIdKhttNhiemVuChiId == _selectedChuongTrinh.Id && s.BIsActive == true && s.ILoai == 1);
                    _itemsPhuongAnNhapKhau = _mapper.Map<ObservableCollection<NhHdnkPhuongAnNhapKhauModel>>(data);
                }
            }
        ...
        {
            MSNTDecisionDialogDetailViewModel.Model = SelectedGoiThau;
            MSNTDecisionDialogDetailViewModel.SoQuetDinhChiTiet = Model.SSoQuyetDinh;
            MSNTDecisionDialogDetailViewModel.Init();
            MSNTDecisionDialogDetailViewModel.ShowDialog();
        }
        ...
        public override void OnSave(object obj)
        {
            ...
            Model.ILoaiQuyetDinh = SelectedDecisionType == null ? 0 : int.Parse(SelectedDecisionType.ValueItem);
            ...
            BackgroundWorkerHelper.Run((s, e) =>
            {
                ...
                var listGoiThau = _mapper.Map<IEnumerable<NhDaGoiThau>>(ItemsGoiThau.IsEmpty() ? new List<NhDaGoiThauModel>() : ItemsGoiThau.Where(s => s.IsChecked) ?? new List<NhDaGoiThauModel>());
                ...
                else
                {
                    ...
                    var listGoiThauNotCheck = _mapper.Map<IEnumerable<NhDaGoiThau>>(ItemsGoiThau.IsEmpty() ? new List<NhDaGoiThauModel>() : ItemsGoiThau.Where(s => !s.IsChecked && s.IIdQuyetDinhChiTietId != null));
                    ...
                    if (!listGoiThau.IsEmpty()) _nhDaGoiThauService.UpdateRange(listGoiThau);
                    if (!listGoiThauNotCheck.IsEmpty()) _nhDaGoiThauService.UpdateRange(listGoiThauNotCheck);
                    LoadDsGoiThau();
                }
                e.Result = Model;
