...
namespace VTS.QLNS.CTC.App.ViewModel.SocialInsurance.SocialInsurancePlan.LapKeHoachChiKhac.ImportKhcK
{
    public class ImportKhcKViewModel : ViewModelBase
    {
        #region Interface
        ...
        public override string Name => "Kế hoạch chi khác ";
        public override Type ContentType => typeof(View.SocialInsurance.SocialInsurancePlan.LapKeHoachChiKhac.ImportKhcK.ImportKhcK);
        public override PackIconKind IconKind => PackIconKind.Dollar;
        private Dictionary<Guid, BhKhcKChiTiet> _dicKhcKChiTiet;
        ...
        private KhcKDetailImportModel _selectedItem;
        public KhcKDetailImportModel SelectedItem
        {
            get => _selectedItem;
        ...
        public bool IsSaveData
        {
            get
            {
                _isSaveData = Items != null && Items.Count > 0 && SelectedDonVi != null && !_importErrors.Any() && _loaiChiSelected != null;
                return _isSaveData;
            }
            set
            {
        ...
        private ObservableCollection<KhcKDetailImportModel> _items;
        public ObservableCollection<KhcKDetailImportModel> Items
        {
            get => _items;
            set
            {
        ...
        private ObservableCollection<KhcKDetailImportModel> _itemsImport;
        public ObservableCollection<KhcKDetailImportModel> ItemsImport
        {
            get => _itemsImport;
            set
            {
        ...
        {
            var soChungTuIndex = _bhKhcKService.GetSoChungTuIndexByCondition(_sessionInfo.YearOfWork);
        ...
        private void HandleData()
        {
            ...
            try
            {
                ...
                if (_importErrors != null && _importErrors.Any())
                {
                    ...
                    {
                        var mucLuc = _nsBhxhMucLucs.FirstOrDefault(x => x.SXauNoiMa.Equals(item.SXauNoiMa) && x.SMoTa.Equals(item.STenMLNS));
                        if (mucLuc != null)
                        {
                ...
                else
                {
                    ...
                    var lstChungTu = _bhKhcKService.FindIndex().Where(x => x.INamLamViec == iNamLamViec);
                    var predicateSummaryDetail = PredicateBuilder.True<BhKhcKChiTiet>();
                    predicateSummaryDetail = predicateSummaryDetail.And(x => x.IID_KHC_K.IsNullOrEmpty());
                    var BhxhChiTiet = _bhKhcKChiTietService.FindAll(predicateSummaryDetail);
                    if (BhxhChiTiet != null)
                    {
                    ...
                    _dicKhcKChiTiet = new Dictionary<Guid, BhKhcKChiTiet>();
                    XlsFile xls = new XlsFile(false);
                    xls.Open(FilePath);
                    xls.ActiveSheet = 1;

                    var dataImport = _importService.ProcessData<KhcKDetailImportModel>(FilePath);
                    ItemsImport = new ObservableCollection<KhcKDetailImportModel>(dataImport.Data.Where(x => x.IsDataNotNull));
                    ...
                    {
                        MessageBoxHelper.Warning(Resources.AlertTypeOfExpenditurePlan);
                        OnResetData();
                        return;
                    }

                    isExist = lstChungTu.Any(x => x.IID_MaDonVi == SelectedDonVi.ValueItem && x.IIDLoaiChi == LoaiChiSelected.Id && x.INamLamViec == _sessionInfo.YearOfWork);
                    if (isExist)
                    {
                    ...
                    var lstLNS = LoaiChiSelected.HiddenValue.Split(',');
                    _nsBhxhMucLucs = _bhDmMucLucNganSachService.FindByCondition(predicate_danhmuc).OrderBy(x => x.SXauNoiMa).Where(x => lstLNS.Contains(x.SLNS)).ToList();

                    List<KhcKDetailImportModel> listDetailImport = ItemsImport.Where(x => x.ImportStatus && !x.IsWarning && x.IsDataNotNull
                    && _nsBhxhMucLucs.Any(y => !y.BHangCha && y.SXauNoiMa == x.SXauNoiMa)).ToList();
                    if (listDetailImport.Count <= 0 || listDetailImport == null)
                    {
                    ...
                    foreach (var item in Items)
                    {
                        ...
                        {
                            if (args.PropertyName == nameof(KhcKDetailImportModel.SXauNoiMa))
                            {
                                var entityDetail = (KhcKDetailImportModel)sender;
        ...
        private IEnumerable<ImportErrorItem> ValidateItem(KhcKDetailImportModel item, int rowIndex)
        {
            try
            {
                List<ImportErrorItem> errors = new List<ImportErrorItem>();
                var listMLNS = _nsBhxhMucLucs.Select(x => x.SXauNoiMa).ToList();
        ...
        private void OnResetData()
        {
            ...
            ItemsImport = new ObservableCollection<KhcKDetailImportModel>();
            Items = new ObservableCollection<KhcKDetailImportModel>();
        ...
        {
            string templateFileName = Path.Combine(ExportPrefix.PATH_BH_KHC_K_IMPORT_TEMPLATE, ExportFileName.IMPORT_BH_KHC_K_CHUNGTU_CHITIET);
        ...
        private void OnSaveData()
        {
            ...
            try
            {
                ...
                _bhKhcKService.Add(chungTu);

                List<BhKhcKChiTiet> chungTuChiTiets = new List<BhKhcKChiTiet>();

                List<KhcKDetailImportModel> listDetailImport = Items.Where(x => x.ImportStatus && !x.IsWarning && x.IsDataNotNull
                && _nsBhxhMucLucs.Any(y => !y.BHangCha && y.SXauNoiMa == x.SXauNoiMa)).ToList();

                foreach (var item in listDetailImport)
                {
                    BhDmMucLucNganSach mucLuc = _nsBhxhMucLucs.FirstOrDefault(x => x.INamLamViec == NamLamViec && x.ITrangThai == StatusType.ACTIVE
                    ...
                    BhKhcKChiTiet bhKhcChiTiet = new BhKhcKChiTiet();
                    bhKhcChiTiet.Id = Guid.NewGuid();
                    bhKhcChiTiet.IID_KHC_K = chungTu.Id;
                ...
                _bhKhcKChiTietService.AddRange(chungTuChiTiets);
                if (chungTuChiTiets.Count > 0)
                {
                    ...
                    _bhKhcKService.Update(chungTu);
                }

                BhKhcKModel khcKhac = _mapper.Map<BhKhcKModel>(chungTu);
                MessageBoxHelper.Info(Resources.MsgSaveDone);
                SavedAction?.Invoke(khcKhac);
            }
            catch (Exception ex)
            {
                _logger.Error(ex.Message, ex);
