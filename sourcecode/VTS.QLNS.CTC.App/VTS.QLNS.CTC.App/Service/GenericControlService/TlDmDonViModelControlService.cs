...
{
    public class TlDmDonViModelControlService : GenericControlBaseService<TlDmDonViModel, TlDmDonVi, TlDmDonViService>
    {
        public override void CustomValueProps(TlDmDonViModel newRow, TlDmDonViModel currentRow)
        {
        ...
        {
            IEnumerable<TlDmDonViModel> models = sourceVM.Items;
            TlDmDonViModel model = sender as TlDmDonViModel;
            if (args.PropertyName == nameof(TlDmDonViModel.IsDeleted))
            {
                TlDmDonViModel_OnDeleted(model, models);
            }
        }

        private void TlDmDonViModel_OnDeleted(TlDmDonViModel model, IEnumerable<TlDmDonViModel> models)
        {
            TlDmDonViModel parent = models.FirstOrDefault(i => i.MaDonVi.Equals(model.ParentId));
            if (parent == null)
            {
                return;
            }
        ...
        public override void OnAddChild(object obj)
        {
            if (sourceVM.SelectedItem == null)
                return;
            TlDmDonViService mucLucNganSachService = sourceVM._service;
            DataGrid dgdData = obj as DataGrid;
            this.CancelEditData(dgdData);

            int currentRow = sourceVM.Items.IndexOf(sourceVM.SelectedItem);
            TlDmDonViModel parent = sourceVM.SelectedItem;
            TlDmDonViModel newRow = new TlDmDonViModel();
            newRow.Id = Guid.Empty;
        ...
        {
            if (property.Name.Equals(nameof(TlDmDonViModel.TenDonViCha)))
            {
                var TlDmDonViService = sourceVM._serviceProvider.GetService(typeof(TlDmDonViService));
                GenericControlCustomViewModel<TlDmDonViModel, TlDmDonVi, TlDmDonViService> dialogVM = new GenericControlCustomViewModel<TlDmDonViModel, TlDmDonVi, TlDmDonViService>
                    ((TlDmDonViService)TlDmDonViService, sourceVM._mapper, sourceVM._sessionService, sourceVM._serviceProvider);
                ...
                {
                    TlDmDonViModel data = obj as TlDmDonViModel;
