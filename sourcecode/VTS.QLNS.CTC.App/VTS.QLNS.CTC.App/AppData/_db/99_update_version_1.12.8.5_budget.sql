/****** Object:  StoredProcedure [dbo].[sp_rpt_skt_chi_tiet_so_nhu_cau_phu_luc_3]    Script Date: 26/05/2023 6:13:54 PM ******/
...
LEFT JOIN  ( SELECT  ctct.iID_MLSKT,       ctct.sKyHieu,
			   SUM (ctct.fTuChi ) AS  fTuChi ,
			   SUM (ctct.fHuyDongTonKho ) AS  fHuyDongTonKho ,
			   SUM (ctct.fTonKhoDenNgay ) AS  fTonKhoDenNgay ,
			   SUM (ctct.fMuaHangCapHienVat ) AS  fMuaHangCapHienVat ,
			   SUM (ctct.fPhanCap ) AS  fPhanCap ,
			   (case WHEN  @loaiNNS = 0  THEN '' ELSE  ctct.sGhiChu END )  AS  sGhiChu
			FROM     #SoNhuCauTongHop ctct 
			GROUP BY  ctct.iID_MLSKT,       ctct.sKyHieu,  ctct.sGhiChu
	   ) snc 



ON snc.iID_MLSKT = ml.iID_MLSKT
