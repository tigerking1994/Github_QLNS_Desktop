/****** Object:  StoredProcedure [dbo].[sp_skt_update_total_dutoandaunam_2]    Script Date: 15/11/2023 10:26:43 AM ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_skt_update_total_dutoandaunam_2]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[sp_skt_update_total_dutoandaunam_2]
GO
/****** Object:  StoredProcedure [dbo].[sp_skt_rpt_dutoandaunam_tonghop]    Script Date: 15/11/2023 10:26:43 AM ******/
...
GO
/****** Object:  UserDefinedFunction [dbo].[f_skt_dulieu_loainns]    Script Date: 15/11/2023 10:26:43 AM ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[f_skt_dulieu_loainns]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
DROP FUNCTION [dbo].[f_skt_dulieu_loainns]
GO
/****** Object:  UserDefinedFunction [dbo].[f_skt_dulieu_loainns]    Script Date: 15/11/2023 10:26:43 AM ******/
SET ANSI_NULLS ON
GO
...
GO
CREATE FUNCTION [dbo].[f_skt_dulieu_loainns] (
@NamLamViec int,
...
FROM
 (SELECT 
  ...
  UNION ALL SELECT 
          ...
          QuyetToan = 0,
		  UocThucHien = 0
  FROM NS_DTDauNam_ChungTuChiTiet_CanCu cancu      
  INNER JOIN NS_DTDauNam_ChungTu as chungtu 
  ON cancu.iID_CTDTDauNam = chungtu.iID_CTDTDauNam
  AND chungtu.iNamLamViec = cancu.iNamLamViec
  AND (@iLoaiNNS = 0 OR chungtu.iLoaiNguonNganSach = @iLoaiNNS)
  WHERE 
     EXISTS (SELECT * FROM TABLE_A)
	 AND cancu.iLoaiChungTu = @LoaiChungTu
	 AND iID_CanCu IN (SELECT iID_CauHinh_CanCu FROM TABLE_A)
     AND (@id_donvi IS NULL OR cancu.iID_MaDonVi IN (SELECT * FROM f_split(@id_donvi)))
  GROUP BY sXauNoiMa,sLNS, sL,sK,sM, sTM, sTTM, sNG, cancu.sMoTa, cancu.iID_MaDonVi
...
FROM
 (SELECT 
  ...
  UNION ALL SELECT LNS, L, K, M, TM, TTM, NG,
          ...
          TuChi = 0,
		  UocThucHien = 0
  FROM f_skt_dulieu_loainns(@NamLamViec, @IdDonvi, @iLoaiNNS, @LoaiChungTu)) AS dt
...
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[sp_skt_update_total_dutoandaunam_2]
...
SET @TongDuToan =
  (SELECT (CASE
   ...
   AND (@iLoaiNNS = 0 OR chungtu.iLoaiNguonNganSach = @iLoaiNNS)
   WHERE chitiet.iNamLamViec = @NamLamViec
     AND chitiet.iID_CTDTDauNam = @ChungTuId
     AND chitiet.iNamNganSach = @NamNganSach
	 AND chitiet.iID_MaNguonNganSach = @NguonNganSach
     AND chitiet.iLoai <> 0
     AND chitiet.iLoaiChungTu = @LoaiChungTu
