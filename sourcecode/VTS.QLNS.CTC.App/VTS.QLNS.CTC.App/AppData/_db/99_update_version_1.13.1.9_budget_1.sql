/****** Object:  StoredProcedure [dbo].[sp_dc_chungtu_chitiet]    Script Date: 26/09/2023 9:51:06 AM ******/
...
GO
/****** Object:  StoredProcedure [dbo].[sp_dutoan_rpt_get_target_donvi]    Script Date: 26/09/2023 2:43:06 PM ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_dutoan_rpt_get_target_donvi]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[sp_dutoan_rpt_get_target_donvi]
GO
/****** Object:  StoredProcedure [dbo].[sp_dc_chungtu_chitiet]    Script Date: 26/09/2023 9:51:06 AM ******/
SET ANSI_NULLS ON
GO
...
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[sp_dutoan_rpt_get_target_donvi]
	 @NamLamViec int,
	 @NamNganSach int,
	 @NguonNganSach int,
	 @LoaiChungTu int,
...
BEGIN 
	...
	SELECT DISTINCT 
		DonVi.*
	FROM 
		NS_DT_ChungTu chungtu
	INNER JOIN 
		NS_DT_ChungTuChiTiet chitiet 
		ON chungtu.iID_DTChungTu = chitiet.iID_DTChungTu
	INNER JOIN
	  (
	   SELECT *
	   FROM DonVi
	  ...
	  ON donvi.iID_MaDonVi = chitiet.iID_MaDonVi
	WHERE chungtu.iNamLamViec = @NamLamViec
		AND chitiet.iNamLamViec = @NamLamViec
		AND chungTu.iLoai = 1
		AND ((@LoaiChungTu = 0) OR (chungTu.iLoaiChungTu = @LoaiChungTu))
		AND chungTu.iID_DTChungTu in (select * from f_split(@IdChungTu));
	--DROP TABLE #tempIds;
END
;
;
