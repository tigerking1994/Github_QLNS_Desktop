/****** Object:  StoredProcedure [dbo].[sp_vdt_get_hop_dong_info]    Script Date: 18/08/2022 8:44:53 AM ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_vdt_get_hop_dong_info]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[sp_vdt_get_hop_dong_info]
GO
/****** Object:  StoredProcedure [dbo].[sp_cp_chungtu_chitiet]    Script Date: 18/08/2022 8:44:53 AM ******/
...
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[sp_vdt_get_hop_dong_info]
	 @NamLamViec int
AS
BEGIN 
	SET NOCOUNT ON;
	SELECT 
		...
		da.sTenDuAn,
		gt.iID_GoiThauID as IdGoiThau,
		gt.sTenGoiThau,
		nhathau.sTenNhaThau,
		loaihopdong.sTenLoaiHopDong,
		dv.sTenDonVi as TenDonVi,
		chudautu.sTenDonVi as ChuDauTu
		INTO #tmp
	FROM
		VDT_DA_TT_HopDong hd
	LEFT JOIN (select * FROM VDT_DA_DuAn) da 
		ON hd.iID_DuAnID = da.iID_DuAnID
	LEFT JOIN (select * FROM  DonVi where iNamLamViec = @NamLamViec) dv 
		ON da.iID_MaDonViThucHienDuAnID = dv.iID_MaDonVi
	LEFT JOIN (select * FROM DM_ChuDauTu where iNamLamViec = @NamLamViec) chudautu 
		ON da.iID_MaChuDauTuID = chudautu.iID_MaDonVi
	LEFT JOIN (select * FROM VDT_DA_GoiThau) gt
		ON hd.iID_GoiThauID = gt.iID_GoiThauID
	LEFT JOIN (select * FROM VDT_DM_NhaThau) nhathau 
		ON hd.iID_NhaThauThucHienID = nhathau.Id
	LEFT JOIN (select * FROM VDT_DM_LoaiHopDong) loaihopdong
		ON hd.iID_LoaiHopDongID = loaihopdong.iID_LoaiHopDongID
	ORDER BY hd.dDateCreate desc
	
	--SELECT tmp.Id, SUM((CASE WHEN tmp.bIsGoc = 1 THEN tmp.fTienHopDong ELSE cd.fTienHopDong END)) AS fGiaTriSauDieuChinh INTO #tmpGiaTriSauDC
	--FROM #tmp AS tmp
	--LEFT JOIN VDT_DA_TT_HopDong AS cd ON tmp.iID_HopDongGocID = cd.iID_HopDongGocID
	--GROUP BY tmp.Id
	SELECT tmp.Id, tmp.fTienHopDong AS fGiaTriSauDieuChinh INTO #tmpGiaTriSauDC
	FROM #tmp AS tmp

	SELECT pr.Id, COUNT(cd.Id) AS iTongSoDieuChinh INTO #tmpDieuChinh
	FROM #tmp AS pr
	INNER JOIN  VDT_DA_TT_HopDong AS cd ON pr.iID_HopDongGocID = cd.iID_HopDongGocID AND cd.Id <> cd.iID_HopDongGocID
	GROUP BY pr.Id

	SELECT  
		tmp.Id,
		tmp.iID_HopDongGocID as IdHopDongGoc,
		tmp.sSoHopDong as SoHopDong,
		tmp.dNgayHopDong as NgayHopDong,
		tmp.IdDuAn,
		tmp.sTenDuAn as TenDuAn,
		tmp.IdGoiThau,
		tmp.sTenGoiThau as TenGoiThau,
		tmp.sTenHopDong as TenHopDong,
		isnull(tmpGiaTriSauDC.fGiaTriSauDieuChinh, cast(0 as float)) as GiaTriSauDieuChinh,
		isnull(cast(tmpDieuChinh.iTongSoDieuChinh as float), cast(0 as float)) as DieuChinh,
		tmp.dDateCreate as NgayTao,
		tmp.TenDonVi,
		tmp.NoiDungHopDong,
		tmp.sTenNhaThau as TenNhaThau,
		tmp.iThoiGianThucHien as ThoiGianThucHien,
		tmp.dKhoiCongDuKien as KhoiCongDuKien,
		tmp.dKetThucDuKien as KetThucDuKien,
		tmp.sHinhThucHopDong as HinhThucHopDong,
		tmp.sTenLoaiHopDong as TenLoaiHopDong,
		tmp.ChuDauTu,
		tmp.sSoTaiKhoan as SSoTaiKhoan,
		tmp.bActive as Active,
		tmp.sUserCreate as SUserCreate,
		tmp.bKhoa as BKhoa,
		isnull(tmp.bIsGoc, cast(0 as bit)) as bIsGoc,
		tmp.iLanDieuChinh as iLanDieuChinh,
		(SELECT COUNT(Id) FROM Attachment WHERE ModuleType = 208 AND ObjectId = tmp.Id) AS TotalFiles
	FROM #tmp AS tmp
	LEFT JOIN #tmpGiaTriSauDC AS tmpGiaTriSauDC ON tmp.Id = tmpGiaTriSauDC.Id
	LEFT JOIN #tmpDieuChinh AS tmpDieuChinh ON tmp.Id = tmpDieuChinh.Id
