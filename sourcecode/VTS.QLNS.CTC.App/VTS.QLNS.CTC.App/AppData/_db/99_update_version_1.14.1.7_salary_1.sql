/****** Object:  StoredProcedure [dbo].[sp_bhxh_lay_luong_ke_hoach]    Script Date: 3/20/2024 6:31:38 PM ******/
...
declare @TBL_LDHD table (TenPhuCap nvarchar(20), MaPhuCap nvarchar(200), GiaTri float);
		...
		WHERE
			Nam = @NamLamViec
			AND kh.parent IN (SELECT * FROM splitstring(@lstIdChungTus))
			AND ((Ma_CB in ('3.1', '3.2', '3.3', '413','414') AND Ma_PhuCap in (@LuongChinh, @PhuCapCV, @PhuCapTNN, @PhuCapTNVK)))) LKH
		GROUP BY
			LKH.Ma_PhuCap,
			LKH.Nam
		...
		WHERE
			Nam = @NamLamViec
			AND kh.parent IN (SELECT * FROM splitstring(@lstIdChungTus))
			AND ((Ma_CB in ('43', '423', '425') AND Ma_PhuCap in (@LuongChinh, @PhuCapCV, @PhuCapTNN, @PhuCapTNVK)))) LKH
		GROUP BY
			LKH.Ma_PhuCap,
			LKH.Nam;
