/****** Object:  StoredProcedure [dbo].[sp_rpt_phanbo_skt_mucluc_don_vi]    Script Date: 10/19/2023 5:13:22 PM ******/
...
GROUP BY iID_MLSKT,
		 ...
		 ---DuToanDuocGiao n -1
		 SELECT  iID_MaDonVi,sSKT_KyHieu,SUM(ISNULL(fUocThucHien,0))/@dvt as DuToan INTO #tmpDutoan 
		...
		where idDonVi Not in (select iID_MaDonVi from DonVi where iLoai = 0 and iNamLamViec =@NamLamViec and iTrangThai =1);
		 ...
		 INSERT INTO #tmpDataEnd(STT, IIdMlskt, IIdMlsktCha, BHangCha, iLoai, idDonVi, sTenDonVi, sMoTa, sKyHieu, sKyHieuCu, sNG, sNgCha, TuChiSoNhuCau, TuChiSoKiemTraDeXuat, TuChiSoKiemTraNamTruoc, DuToan)
		( SELECT 
				mlnsCha.sSTT,
				mlnsCha.iID_MLSKT,
		 ...
		 With #TreeEndOut(STT, IIdMlskt, IIdMlsktCha, BHangCha, iLoai, idDonVi, sTenDonVi, sMoTa, sKyHieu,sKyHieuCu, sNG, sNgCha, TuChiSoNhuCau, TuChiSoKiemTraDeXuat, TuChiSoKiemTraNamTruoc, FDuToan, position)
		 AS
		 (
