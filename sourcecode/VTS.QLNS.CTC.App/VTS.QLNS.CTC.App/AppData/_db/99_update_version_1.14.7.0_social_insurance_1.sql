/****** Object:  StoredProcedure [dbo].[sp_rpt_bhxh_luong_tro_cap_om_dau]    Script Date: 7/31/2024 9:15:32 AM ******/
...
GO
/****** Object:  StoredProcedure [dbo].[sp_rpt_bhxh_giaithich_trocap]    Script Date: 7/31/2024 9:15:32 AM ******/
...
GO
/****** Object:  StoredProcedure [dbo].[sp_bhxh_qttm_get_data_quy]    Script Date: 7/31/2024 9:15:32 AM ******/
...
BEGIN
	...
	select
		...
		sum(isnull(ctct.fSoPhaiThu, 0)) fSoPhaiThu
	...
	IF(@TypeOutPut = 2)
		...
		--Lấy Đơn vị
			...
			SELECT 
				...
				--OUTPUT---
			SELECT rs.* INTO #resuld_tbl_2 FROM
			(
			...
			--
			select row_number() over(partition by MaDonVi order by STT) as row_rank, STT STT_R, MaDonVi MaDonVi_R
			into #TBL_RANK_2
			from #resuld_tbl_2
			where Level = 1 and (Total <> 0 or Total is not null)
			--
			update #resuld_tbl_2
			set STT = (select r.row_rank from #TBL_RANK_2 r where r.STT_R = STT and r.MaDonVi_R = MaDonVi)
			where Level = 1 and (Total <> 0 or Total is not null)
			--
			SELECT * FROM #resuld_tbl_2 ORDER BY MaDonVi, rowNumber, MaCanBo, STT
...
-- Lấy Loại đối tượng
			...
			where Level = 1 and (Total <> 0 or Total is not null)
			--
			SELECT * FROM #resuld_tbl ORDER BY rowNumber, MaCanBo, STT
...
BEGIN
	...
	(select
		1 bHangCha, 1 RowNum, 'I' STT, 'SQ' DoiTuong, 'SQ' LoaiDoiTuong, null Ma_DonVi, null TenDonVi, null sMaCB, null sMaCBo, 'Sĩ quan' sTenCbo, null SoNgayBenhDaiNgayD14Ngay, null SoNgayBenhDaiNgayT14Ngay, null SoNgayOmKhacD14Ngay, null SoNgayOmKhacT14Ngay, null SoNgayConOm, null SoNgayDuongSuc, null fLuongCanCu, null fBENHDAINGAY_D14NGAY, null fBENHDAINGAY_T14NGAY, null fOMKHAC_D14NGAY, null fOMKHAC_T14NGAY, null fCONOM, null fDUONGSUCPHSK, 0 bHasData
	union all
	select
	...
	(select
		1 bHangCha, 4 RowNum, 'IV' STT, 'VCQP' DoiTuong, 'VCQP' LoaiDoiTuong, null Ma_DonVi, null TenDonVi, null sMaCB, null sMaCBo, 'CNVCQP' sTenCbo, null SoNgayBenhDaiNgayD14Ngay, null SoNgayBenhDaiNgayT14Ngay, null SoNgayOmKhacD14Ngay, null SoNgayOmKhacT14Ngay, null SoNgayConOm, null SoNgayDuongSuc, null fLuongCanCu, null fBENHDAINGAY_D14NGAY, null fBENHDAINGAY_T14NGAY, null fOMKHAC_D14NGAY, null fOMKHAC_T14NGAY, null fCONOM, null fDUONGSUCPHSK, 0 bHasData
	union all
	select
