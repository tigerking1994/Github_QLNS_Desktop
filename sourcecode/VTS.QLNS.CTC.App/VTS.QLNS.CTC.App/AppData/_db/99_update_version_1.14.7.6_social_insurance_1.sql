/****** Object:  StoredProcedure [dbo].[sp_bhxh_qtt_get_data_thang_quy_donvi_cha]    Script Date: 9/4/2024 2:50:18 PM ******/
...
GO
/****** Object:  StoredProcedure [dbo].[sp_bhxh_qtt_get_data_thang_quy_donvi]    Script Date: 9/4/2024 2:50:18 PM ******/
...
BEGIN
	...
	declare @IThang int
	declare @IThangQuy int
	declare @SoThangQuy int
	declare @LoaiThangQuy int

	--check có chứng từ tháng ko
	set @IThang = (select count(distinct sSoChungTu) from BH_QTT_BHXH_ChungTu where iID_MaDonVi = @IdMaDonVi and iNamLamViec = @INamLamViec and bIsKhoa = 1 and iQuyNamLoai = 0)
	--check có chứng từ tháng, quý ko
	set @IThangQuy = (select count(distinct sSoChungTu) from BH_QTT_BHXH_ChungTu where iID_MaDonVi = @IdMaDonVi and iNamLamViec = @INamLamViec and bIsKhoa = 1 and iQuyNamLoai in (0, 1))
	
	if (@IThang > 0)
		begin
		set @SoThangQuy = (select count(distinct sSoChungTu) from BH_QTT_BHXH_ChungTu where iID_MaDonVi = @IdMaDonVi and iNamLamViec = @INamLamViec and bIsKhoa = 1 and iQuyNamLoai = 0)
		set @LoaiThangQuy = 0
		end
	else
		begin
		set @SoThangQuy = (select count(distinct sSoChungTu) from BH_QTT_BHXH_ChungTu where iID_MaDonVi = @IdMaDonVi and iNamLamViec = @INamLamViec and bIsKhoa = 1 and iQuyNamLoai = 1)
		set @LoaiThangQuy = 1
		end
	...
	select
		...
		ct.iID_MaDonVi,
		ct.iLoai, 
		case when @IThangQuy > 0 then round((cast(isnull(sum(ctct.iQSBQNam), 0) as float) / @SoThangQuy), 0)
