/****** Object:  StoredProcedure [dbo].[sp_rpt_tong_hop_quyet_toan_thu_chi]    Script Date: 07/12/2023 6:54:52 PM ******/
...
GO
/****** Object:  StoredProcedure [dbo].[sp_rpt_tong_hop_du_toan_thu_chi]    Script Date: 07/12/2023 6:54:52 PM ******/
...
ON ctct.iID_MucLucNganSach = ml.iID_MLNS AND ct.iNamChungTu = ml.iNamLamViec
	AND ml.iTrangThai = 1
	--AND ml.sLNS = '9020001'
	--AND ml.sM = ''
INNER JOIN 
...
INNER JOIN BH_DM_MucLucNganSach ml 
ON ctct.iID_NoiDung = ml.iID_MLNS AND ct.iNamChungTu = ml.iNamLamViec
	AND ml.iTrangThai = 1
	--AND ml.sLNS = '9020001'
	--AND ml.sM = ''
INNER JOIN 
...
FROM   
   (SELECT 
		SUM(ISNULL(fSoTienThuBHXH, 0)) AS N'1.1.0,1,Dự toán thu BHXH',
		SUM(ISNULL(fSoTienThuBHTN, 0)) AS N'1.2.0,2,Dự toán thu BHTN',
		SUM(ISNULL(fSoTienThuBHYT_QN, 0)) AS N'1.3.0,3,Dự toán thu BHYT quân nhân',
		SUM(ISNULL(fSoTienThuBHYT_NLD, 0)) AS N'1.4.0,4,Dự toán thu BHYT người lao động'
   FROM #temp1) p  
UNPIVOT  
...
FROM   
   (SELECT 
		SUM(ISNULL(fSoTienThuBHYT_TNQN, 0)) AS N'1.5.1,-,Thân nhân quân nhân',
		SUM(ISNULL(fSoTienThuBHYT_CNVQP, 0)) AS N'1.5.2,-,Thân nhân người lao động'
...
FROM   
   (SELECT 
		SUM(ISNULL(fSoTienChiCheDo, 0)) AS N'2.1.0,1,Dự toán chi các chế độ BHXH'
   FROM #temp3) p  
UNPIVOT  
   (SoTien FOR NoiDung IN   
...
FROM   
   (SELECT 
		SUM(ISNULL(fSoTienChiKinhPhiQuanLy, 0)) AS N'2.2.0,2,Dự toán chi kinh phí quản lý BHXH, BHYT'
   FROM #temp4) p  
UNPIVOT  
   (SoTien FOR NoiDung IN   
...
FROM   
   (SELECT 
		SUM(ISNULL(fSoTienChiQuanYDonVi, 0)) AS N'2.3.0,3,Dự toán chi kinh phí KCB tại quân y đơn vị'
   FROM #temp5) p  
UNPIVOT  
   (SoTien FOR NoiDung IN   
...
FROM   
   (SELECT 
		SUM(ISNULL(fSoTienThuBHXH, 0)) AS N'1.1.0,1,Thu bảo hiểm xã hội (Phụ lục II)',
		SUM(ISNULL(fSoTienThuBHTN, 0)) AS N'1.2.0,2,Thu bảo hiểm thất nghiệp (Phụ lục III)',
		SUM(ISNULL(fSoTienThuBHYT_QN, 0)) AS N'1.3.1,-,BHYT quân nhân (Phụ lục IV)',
		SUM(ISNULL(fSoTienThuBHYT_NLD, 0)) AS N'1.3.2,-,BHYT người lao động (Phụ lục V)'
   FROM #temp1) p  
UNPIVOT  
   (SoTien FOR NoiDung IN   
...
FROM   
   (SELECT 
		SUM(ISNULL(fSoTienThuBHYT_TNQN, 0)) AS N'1.3.3,-,BHYT thân nhân quân nhân (Phụ lục VI)',
		SUM(ISNULL(fSoTienThuBHYT_CNVQP, 0)) AS N'1.3.4,-,BHYT thân nhân CN, viên chức QP (Phụ lục VI)',
		SUM(ISNULL(fSoTienThuBHYT_HVQS, 0)) AS N'1.3.7,-,BHYT HV QS xã phường (Phụ lục VII)',
		SUM(ISNULL(fSoTienThuBHYT_HSSV, 0)) AS N'1.3.5,-,BHYT học sinh, sinh viên (Phụ lục VII)',
		SUM(ISNULL(fSoTienThuBHYT_SQDB, 0)) AS N'1.3.8,-,BHYT SQ dự bị (Phụ lục VII)',
		SUM(ISNULL(fSoTienThuBHYT_LUU_HS, 0)) AS N'1.3.6,-,BHYT lưu học sinh (Phụ lục VII)'
...
FROM   
   (SELECT 
		SUM(ISNULL(fSoTienChiCheDo, 0)) AS N'2.1.0,1,Chi các chế độ BHXH (Phụ lục VIII)'
   FROM #temp3) p  
UNPIVOT  
   (SoTien FOR NoiDung IN   
...
FROM   
   (SELECT 
		SUM(ISNULL(fSoTienChiKinhPhiQuanLy, 0)) AS N'2.2.0,2,Chi KP quản lý BHXH, BHYT (Phụ lục IX)'
   FROM #temp4) p  
UNPIVOT  
   (SoTien FOR NoiDung IN   
...
FROM   
   (SELECT 
		SUM(ISNULL(fSoTienChiQuanYDonVi, 0)) AS N'2.4.2,-,Chi KCB tại quân y đơn vị (Phụ lục XII)'
   FROM #temp5) p  
UNPIVOT  
   (SoTien FOR NoiDung IN   
...
FROM   
   (SELECT 
		SUM(ISNULL(fSoTienChiTTB, 0)) AS N'2.3.0,3,Chi mua sắm TTB y tế (Phụ lục X)',
		SUM(ISNULL(fSoTienChiTSDK, 0)) AS N'2.4.1,-,Chi KCB cho quân nhân tại TS-DK (Phụ lục XI)',
		SUM(ISNULL(fSoTienChiNLD, 0)) AS N'2.4.3,-,Chi kinh phí CSSK BĐ người lao động (Phụ lục XIII)',
		SUM(ISNULL(fSoTienChiHSSV, 0)) AS N'2.4.4,-,Chi kinh phí CSSK BĐ HSSV (Phụ lục XIV)',
		SUM(ISNULL(fSoTienChiCSYT, 0)) AS N'2.4.5,-,Chi KCB tại các cơ sở y tế (Phụ lục XV)'
   FROM #temp6) p  
