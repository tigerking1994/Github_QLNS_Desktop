/****** Object:  StoredProcedure [dbo].[sp_dt_quyettoan_mucluccongkhai]    Script Date: 9/22/2023 5:25:07 PM ******/
	...
	tblSoPhanBo AS (
		...
		(
			...
			AND (
			    ...
			    OR (@Time <> 0
					and (
						YEAR(ct.dNgayChungTu) < @YearOfWork 
						or (MONTH(ct.dNgayChungTu) <= @Time and YEAR(ct.dNgayChungTu) = @YearOfWork)
						)
					  )
					)
			)
			...
			AND (
			    ...
			    OR (@Time <> 0
					and (
						YEAR(ct.dNgayChungTu) < @YearOfWork-1 
						or (MONTH(ct.dNgayChungTu) <= @Time and YEAR(ct.dNgayChungTu) = @YearOfWork-1)
						)
					  )
					)
			)
