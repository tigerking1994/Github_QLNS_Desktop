/****** Object:  StoredProcedure [dbo].[sp_rpt_skt_tong_hop_so_nhu_cau_phu_luc_5]    Script Date: 25/05/2022 6:49:47 PM ******/
...
GO
/****** Object:  StoredProcedure [dbo].[sp_rpt_skt_tong_hop_so_nhu_cau_phu_luc_4]    Script Date: 25/05/2022 6:49:47 PM ******/
...
GO
/****** Object:  StoredProcedure [dbo].[sp_rpt_skt_tong_hop_so_nhu_cau_phu_luc_3]    Script Date: 25/05/2022 6:49:47 PM ******/
...
GO
/****** Object:  StoredProcedure [dbo].[sp_rpt_skt_chi_tiet_so_nhu_cau_phu_luc_5]    Script Date: 25/05/2022 6:49:47 PM ******/
...
GO
/****** Object:  StoredProcedure [dbo].[sp_rpt_skt_chi_tiet_so_nhu_cau_phu_luc_4]    Script Date: 25/05/2022 6:49:47 PM ******/
...
GO
/****** Object:  StoredProcedure [dbo].[sp_rpt_skt_chi_tiet_so_nhu_cau_phu_luc_3]    Script Date: 25/05/2022 6:49:47 PM ******/
...
FROM NS_SKT_MucLuc ml
LEFT JOIN #SoNhuCauTongHop snc on snc.iID_MLSKT = ml.iID_MLSKT 
LEFT JOIN @tblSkt skt on skt.iID_MLSKT = ml.iID_MLSKT 
WHERE ml.iNamLamViec = @NamLamViec
...
declare @tblSkt table (iID_MLSKT uniqueidentifier, TuChi float, MuaHangHienVat float, DacThu float)
declare @countCanCuDuToan int;
declare @countCanCuSkt int ;
declare @countSktChiTiet int ; 
declare @countDonVi int;
...
ELSE
INSERT INTO @tblDuToan (iID_MLSKT, KyHieu, TuChi, MuaHangHienVat, DacThu)
SELECT cc.iID_MLSKT, null, sum(isnull(cc.fTuChi,0)), sum(isnull(cc.fMuaHangCapHienVat,0)), sum(isnull(cc.fPhanCap,0))
FROM 
	(SELECT * 
	FROM NS_SKT_ChungTuChiTiet_CanCu 
	WHERE 
...
WHERE ct.iNamLamViec = @NamLamViec - 1
  ...
  ELSE
INSERT INTO @tblSkt (iID_MLSKT,TuChi, MuaHangHienVat, DacThu)
SELECT cc.iID_MLSKT, sum(isnull(cc.fTuChi,0)), sum(isnull(cc.fMuaHangCapHienVat,0)), sum(isnull(cc.fPhanCap,0))
FROM 
	(SELECT * 
	FROM NS_SKT_ChungTuChiTiet_CanCu 
	WHERE 
...
LEFT JOIN #SoNhuCauTongHop snc on snc.iID_MLSKT = ml.iID_MLSKT
LEFT JOIN @tblSkt skt on skt.iID_MLSKT = ml.iID_MLSKT
LEFT JOIN @tblDuToan dt on ((@countSktChiTiet != 0 or @countCanCuDuToan != 0) AND dt.iID_MLSKT = ml.iID_MLSKT) OR (@countSktChiTiet = 0 and @countCanCuDuToan = 0 AND dt.KyHieu = ml.sKyHieu)
WHERE ml.iNamLamViec = @NamLamViec
AND (@MaBQuanLy = '0' OR ml.sKyHieu in (SELECT * FROM #KyHieuSktBQuanLy))
and (skt.MuaHangHienVat <> 0 or dt.MuaHangHienVat <> 0 or snc.fHuyDongTonKho <> 0 or snc.fMuaHangCapHienVat <> 0 or (snc.sGhiChu is not null and snc.sGhiChu != ''));
