/****** Object:  StoredProcedure [dbo].[sp_rpt_phanbo_skt_mucluc_don_vi]    Script Date: 11/15/2024 3:29:49 PM ******/
...
-----SKT nam truoc
	SELECT iID_MLSKT IIdMlskt,
       iID_MLSKTCha IIdMlsktCha,
		(Case
			When dt_dv.id is null Then A.iID_MaDonVi
			else dt_dv.id
		ENd) as idDonVi,
		(Case
			When dt_dv.sTenDonVi is null Then A.sTenDonVi
			else dt_dv.sTenDonVi
		ENd) as sTenDonVi,
		(Case
			When dt_dv.iLoai is null Then 1
			else dt_dv.iLoai
		ENd) as iLoai,
       --dt_dv.iLoai,
       sKyHieu,
...
GROUP BY iID_MLSKT,
		 ...
		 ---DuToanDuocGiao n -1
		 SELECT  iID_MaDonVi,map.sSKT_KyHieu,SUM(ISNULL(fTuChi,0))/@dvt as DuToan INTO #tmpDutoan 		 
		 ...
		 select 
				(Case
					When namNay.STT is null Then sktNamTruoc.STT
					else namNay.STT
				ENd) as STT,
				(Case
					When namNay.IIdMlskt is null Then sktNamTruoc.IIdMlskt
					else namNay.IIdMlskt
				ENd) as IIdMlskt,
				(Case
					When namNay.IIdMlsktCha is null Then sktNamTruoc.IIdMlsktCha
					else namNay.IIdMlsktCha
				ENd) as IIdMlsktCha,
				(Case
					When namNay.BHangCha is null Then sktNamTruoc.BHangCha
					else namNay.BHangCha
				ENd) as BHangCha,
				(Case
					When namNay.iLoai is null Then sktNamTruoc.iLoai
					else namNay.iLoai
				ENd) as iLoai,
				(Case
					When namNay.idDonVi is null Then sktNamTruoc.idDonVi
					else namNay.idDonVi
				ENd) as idDonVi,
				(Case
					When namNay.sTenDonVi is null Then sktNamTruoc.sTenDonVi
					else namNay.sTenDonVi
				ENd) as sTenDonVi,
				--namNay.sTenDonVi,
				(Case
					When namNay.sMoTa is null Then sktNamTruoc.sMoTa
					else namNay.sMoTa
				ENd) as sMoTa,
				(Case
					When namNay.sKyHieuCu is null Then sktNamTruoc.sKyHieu
					else namNay.sKyHieuCu
				ENd) as sKyHieu,
				(Case
					When namNay.sKyHieuCu is null Then sktNamTruoc.sKyHieuCu
					else namNay.sKyHieuCu
				ENd) as sKyHieuCu,
				--namNay.sKyHieuCu,
				(Case
					When namNay.sNG is null Then sktNamTruoc.sNG
					else namNay.sNG
				ENd) as sNG,
				(Case
					When namNay.sNgCha is null Then sktNamTruoc.sNgCha
					else namNay.sNgCha
				ENd) as sNgCha,
				--namNay.sNgCha,
				...
				namNay.GhiChu,
				(Case
					When namNay.IIdMlskt is null Then 0
					else 1
				ENd) as LoaiNamNay
				INTO #tmpOutPut
		 FROM #tmpSKtNamNay namNay
		 FULL JOIN #tmpSKtNamTruoc sktNamTruoc on namNay.sKyHieuCu = sktNamTruoc.sKyHieu and namNay.idDonVi = sktNamTruoc.idDonVi
		...
		where idDonVi Not in (select iID_MaDonVi from DonVi where iLoai = 0 and iNamLamViec =@NamLamViec and iTrangThai =1);
		 ...
		 INSERT INTO #tmpDataEnd(STT, IIdMlskt, IIdMlsktCha, BHangCha, iLoai, idDonVi, sTenDonVi, sMoTa, sKyHieu, sKyHieuCu, sNG, sNgCha, TuChiSoNhuCau, TuChiSoKiemTraDeXuat, TuChiSoKiemTraNamTruoc, DuToan,GhiChu,LoaiNamNay)
		( SELECT 
				mlnsCha.sSTT,
				mlnsCha.iID_MLSKT,
		 ...
		 With #TreeEndOut(STT, IIdMlskt, IIdMlsktCha, BHangCha, iLoai, idDonVi, sTenDonVi, sMoTa, sKyHieu,sKyHieuCu, sNG, sNgCha, TuChiSoNhuCau, TuChiSoKiemTraDeXuat, TuChiSoKiemTraNamTruoc, FDuToan,GhiChu, LoaiNamNay,position)
		 AS
		 (
			SELECT *, CAST(ROW_NUMBER() OVER(ORDER BY STT) AS NVARCHAR(MAX)) AS position 
			FROM #tmpDataEnd 
			WHERE isnull(IIdMlsktCha, '00000000-0000-0000-0000-000000000000') = '00000000-0000-0000-0000-000000000000'
			UNION ALL
			SELECT child.*, CONCAT(parent.position,'.',CAST(ROW_NUMBER() OVER(ORDER BY child.sNG,child.sKyHieu,child.idDonVi) AS NVARCHAR(MAX))) AS position 
